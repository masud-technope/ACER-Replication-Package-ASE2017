...
@Properties({ @Property(name = "service.description", value = "Sling Object Content Manager Factory"), @Property(name = "service.vendor", value = "The Apache Software Foundation") })
public class ObjectContentManagerFactoryImpl implements ObjectContentManagerFactory, SynchronousBundleListener {
    ...
    public ObjectContentManager getObjectContentManager(Session session) {
        ...
        try {
            ...
            log.info("getObjectContentManager: Cannot get ValueFactory from Session (" + session.getUserID() + "), using default factory", re);
            valueFactory = ValueFactoryImpl.getInstance();
        }
        ObjectCache objectCache = new ObservingObjectCache(session);
        QueryManager queryManager = new QueryManagerImpl(mapper, converterProvider.getAtomicTypeConverters(), valueFactory);
        ObjectConverter objectConverter = new ObjectConverterImpl(mapper, converterProvider, new ProxyManagerImpl(), objectCache);
        return new ObjectContentManagerImpl(mapper, objectConverter, queryManager, objectCache, session);
    }
    ...
    protected void activate(ComponentContext componentContext) {
        ...
        String[] mappedClasses = (mapper != null) ? mapper.getMappedClasses() : new String[0];
        adapterFactory = new OcmAdapterFactory(this, componentContext.getBundleContext(), mappedClasses);
