...
public class ResourceProxy {
    ...
    public void addChild(ResourceProxy child) {
        ...
        if (!isParent(path, child.getPath())) {
            throw new IllegalArgumentException("Resource at path " + child.getPath() + " is not a direct child of " + path);
        }
    ...
    public <T> T adaptTo(Class<T> klazz) {
        ...
        @SuppressWarnings("unchecked") T res = (T) adapted.get(klazz);
        return res;
    }
    ...
    private String toString0(int padding) {
        ...
        out.append(getClass().getSimpleName()).append(": path=").append(path).append(", properties=").append(properties);
