...
public abstract class AbstractResourceCollector {
    ...
    public  AbstractResourceCollector(final String baseResourceType, final String resourceType, final String resourceSuperType, final String extension, final String[] executionPaths) {
    ...
    public final Collection<Resource> getServlets(final ResourceResolver resolver) {
        ...
        final Iterator<String> locations = new LocationIterator(resourceType, resourceSuperType, baseResourceType, resolver);
    ...
    protected abstract void getWeightedResources(final Set<Resource> resources, final Resource location);
     ...
     */
    protected final void addWeightedResource(final Set<Resource> resources, final Resource resource, final int numSelectors, final int methodPrefixWeight) {
     ...
     */
    protected final Resource getResource(final ResourceResolver resolver, String path) {
    ...
    public boolean equals(Object obj) {
        ...
        if (stringEquals(resourceType, o.resourceType) && stringEquals(resourceSuperType, o.resourceSuperType) && stringEquals(extension, o.extension) && stringEquals(baseResourceType, o.baseResourceType)) {
            return true;
        }
        return false;
    }
    ...
    public static boolean isPathAllowed(final String path, final String[] executionPaths) {
        if (executionPaths == null || executionPaths.length == 0) {
            SlingServletResolver.LOGGER.debug("Accepting servlet at '{}' as there are no configured execution paths.", path);
            return true;
        }
        ...
        for (final String config : executionPaths) {
            if (config.endsWith("/")) {
                if (path.startsWith(config)) {
                    SlingServletResolver.LOGGER.debug("Accepting servlet at '{}' as the path is prefixed with configured execution path '{}'.", path, config);
                    return true;
                }
            } else if (path.equals(config)) {
                SlingServletResolver.LOGGER.debug("Accepting servlet at '{}' as the path equals configured execution path '{}'.", path, config);
                return true;
            }
        }
