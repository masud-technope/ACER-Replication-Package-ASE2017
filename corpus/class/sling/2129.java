    ...
    private static void appendProperties(final StringBuilder sb, final Map<String, Object> properties) {
        if (properties != null) {
            ...
            for (final String propName : properties.keySet()) {
                if (propName.equals(ResourceHelper.PROPERTY_JOB_ID) || propName.equals(ResourceHelper.PROPERTY_JOB_TOPIC)) {
                    continue;
                }
                if (first) {
                    first = false;
        ...
        if (resource != null) {
            try {
                ...
                if (errorMessage == null && jobId != null) {
                    ...
                    @SuppressWarnings("unchecked") final List<Exception> readErrorList = (List<Exception>) jobProperties.get(ResourceHelper.PROPERTY_MARKER_READ_ERROR_LIST);
