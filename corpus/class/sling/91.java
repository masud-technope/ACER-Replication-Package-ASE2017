...
public class RepositoryImpl implements Repository {
    ...
    @Override
    public Command<Void> newAddOrUpdateNodeCommand(CommandContext context, final FileInfo fileInfo, ResourceProxy resource, CommandExecutionFlag... flags) {
        if (flags.length != 0) {
            throw new UnsupportedOperationException("This implementation does not support any flags");
        }
        return wrap(new UpdateContentCommand(repositoryInfo, httpClient, fileInfo.getRelativeLocation(), resource.getProperties(), fileInfo));
    }
