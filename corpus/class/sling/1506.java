...
@Properties({ @Property(name = Constants.SERVICE_DESCRIPTION, value = "Apache Sling Resource Resolver Factory"), @Property(name = Constants.SERVICE_VENDOR, value = "The Apache Software Foundation") })
@References({ @Reference(name = "ResourceDecorator", referenceInterface = ResourceDecorator.class, cardinality = ReferenceCardinality.OPTIONAL_MULTIPLE, policy = ReferencePolicy.DYNAMIC) })
public class ResourceResolverFactoryActivator implements Runnable {
    ...
    @Property(boolValue = DEFAULT_ENABLE_VANITY_PATH, label = "Enable Vanity Paths", description = "This flag controls whether all resources with a sling:vanityPath property " + "are processed and added to the mappoing table.")
    ...
    @Property(boolValue = DEFAULT_MAX_CACHED_VANITY_PATHS_STARTUP, label = "Limit the maximum number of cached vanity path entries only at startup", description = "Limit the maximum number of cached vanity path entries only at startup. " + "Default is true")
    ...
    @Property(longValue = DEFAULT_VANITY_BLOOM_FILTER_MAX_BYTES, label = "Maximum number of vanity bloom filter bytes", description = "The maximum number of vanity bloom filter bytes. " + "Changing this value is subject to vanity bloom filter rebuild")
    ...
    @Property(boolValue = DEFAULT_VANITY_PATH_PRECEDENCE, label = "Vanity Path Precedence", description = "This flag controls whether vanity paths" + " will have precedence over existing /etc/map mapping")
    ...
    @Property(boolValue = DEFAULT_PARANOID_PROVIDER_HANDLING, label = "Paranoid Provider Handling", description = "If this flag is enabled, an unregistration of a resource provider (not factory), " + "is causing the resource resolver factory to restart, potentially cleaning up " + "for memory leaks caused by objects hold from that resource provider.")
    ...
    @Property(boolValue = DEFAULT_LOG_RESOURCE_RESOLVER_CLOSING, label = "Log resource resolver closing", description = "When enabled CRUD operations with a closed resource resolver will log a stack trace " + "with the point where the used resolver was closed. It's advisable to not enable this feature on " + "production systems.")
    ...
    private enum BG_OP implements  {

        // check preconditions
        CHECK() {
        }
        , // unregister first, then check preconditions
    ...
    protected void activate(final ComponentContext componentContext) {
        ...
        if (this.resourceProviderTracker == null) {
            ...
            this.changeListenerWhiteboard.activate(this.componentContext.getBundleContext(), this.resourceProviderTracker, searchPath);
            this.resourceProviderTracker.activate(this.componentContext.getBundleContext(), this.eventAdmin, new ChangeListener() {
        ...
        this.logResourceResolverClosing = PropertiesUtil.toBoolean(properties.get(PROP_LOG_RESOURCE_RESOLVER_CLOSING), DEFAULT_LOG_RESOURCE_RESOLVER_CLOSING);
    ...
    private void registerFactory(final ComponentContext localContext) {
        ...
        if (localContext != null) {
            ...
            local.factoryRegistration = localContext.getBundleContext().registerService(ResourceResolverFactory.class.getName(), new ServiceFactory() {
            ...
            local.runtimeRegistration = localContext.getBundleContext().registerService(RuntimeService.class.getName(), this.getRuntimeService(), null);
