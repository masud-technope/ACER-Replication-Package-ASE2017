...
@Properties({ @Property(name = "service.description", value = "Sling Filesystem Resource Provider"), @Property(name = "service.vendor", value = "The Apache Software Foundation"), @Property(name = ResourceProvider.ROOTS), @Property(name = "webconsole.configurationFactory.nameHint", value = "Root paths: {" + ResourceProvider.ROOTS + "}") })
public class FsResourceProvider implements ResourceProvider {
    ...
    protected void activate(BundleContext bundleContext, Map<?, ?> props) {
        ...
        if (providerFileName == null || providerFileName.length() == 0) {
            throw new IllegalArgumentException(PROP_PROVIDER_FILE + " property must be set");
        }
