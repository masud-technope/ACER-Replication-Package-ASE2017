...
public class FindPathsToWatchTest extends RepositoryTestBase {
    ...
    public void testInitialFind() throws Exception {
        ...
        for (String folder : contentHelper.WATCHED_FOLDERS) {
            assertTrue("Folder " + folder + " must be watched (watched=" + wf + ")", isWatched(folder, wf));
        }
    }

    public void testNewWatchedFolderDetection() throws Exception {
        final String newPaths[] = { "/libs/tnwf/install", "/libs/tnwf/install.dev", "/apps/tnwf/install", "/apps/tnwf/install.staging", "/apps/tnwf/install.staging.dev", "/NOT/foo/nothing" };
        for (String newPath : newPaths) {
            assertFalse(newPath + " must not be watched before test", isWatched(newPath, MiscUtil.getWatchedFolders(installer)));
            ...
            if (newPath.contains("NOT")) {
                assertFalse(newPath + " must not be watched after test", isWatched(newPath, MiscUtil.getWatchedFolders(installer)));
            } else {
                assertTrue(newPath + " must be watched after calling addNewWatchedFolders()", isWatched(newPath, MiscUtil.getWatchedFolders(installer)));
            }
        }
        ...
        MiscUtil.waitAfterContentChanges(eventHelper, installer);
        assertFalse(folder + " must not be watched anymore after deleting", isWatched(folder, MiscUtil.getWatchedFolders(installer)));
        assertEquals("Expecting only one WatchedFolder to be deleted", nBefore - 1, MiscUtil.getWatchedFolders(installer).size());
    }
