...
class DriverDataSource implements DataSource {
    ...
    public Connection getConnection(String usr, String pwd) throws SQLException {
        ...
        if (usr != null)
            properties.setProperty(PoolUtilities.PROP_USER, usr);
        if (pwd != null)
            properties.setProperty(PoolUtilities.PROP_PASSWORD, pwd);
        ...
        try {
            ...
            if (jmxPool != null) {
                jmxPool.notify(org.apache.tomcat.jdbc.pool.jmx.ConnectionPool.NOTIFY_CONNECT, ConnectionPool.getStackTrace(x));
            }
    ...
    private Driver getDriver() throws SQLException {
        ...
        if (driver == null && poolProperties.getDriverClassName() != null) {
            log.debug("Looking for driver for [{}] via provided className [{}]", url, poolProperties.getDriverClassName());
        ...
        if (driver == null) {
            String msg = String.format("Not able to find any matching driver for url [%s] " + "and driverClassName [%s]", url, poolProperties.getDriverClassName());
    ...
    private Driver loadDriverClass() throws SQLException {
        try {
            log.debug("Instantiating driver using class: {} [url={}]", poolProperties.getDriverClassName(), poolProperties.getUrl());
            return (Driver) bundleContext.getBundle().loadClass(poolProperties.getDriverClassName()).newInstance();
