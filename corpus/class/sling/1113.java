...
public class RepositoryAccessor {
    ...
    public  RepositoryAccessor(LaunchpadConfig config) {
        ...
        client.getState().setCredentials(new AuthScope(config.getHostname(), config.getPort()), new UsernamePasswordCredentials(config.getUsername(), config.getPassword()));
    }
    ...
    public void assertGetIsSuccessful(String path, String expectedResult) throws HttpException, IOException {
        ...
        try {
            ...
            try (InputStream input = m.getResponseBodyAsStream()) {
                ...
                assertThat("Unexpected response for " + m.getURI(), responseBody, CoreMatchers.equalTo(expectedResult));
            }
        } finally {
            m.releaseConnection();
        }
    ...
    public void createFile(String path, byte[] bytes) throws RepositoryException {
        ...
        try {
            ...
            contentNode.setProperty("jcr:data", session.getValueFactory().createBinary(new ByteArrayInputStream(bytes)));
            session.save();
        } finally {
            session.logout();
        }
