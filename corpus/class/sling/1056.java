...
public class SetupServerWizardPage extends WizardPage {
    ...
    private void dialogChanged() {
        ...
        if (useExistingServer.getSelection()) {
            ...
            if (newServerName.getText().length() == 0 || getHostname().length() == 0 || newServerPort.getText().length() == 0 || newServerDebugPort.getText().length() == 0 || newServerUsername.getText().length() == 0 || newServerPassword.getText().length() == 0) {
    ...
    public boolean getStartServer() {
        if (!useExistingServer.getSelection()) {
            // new servers are automatically started
            return true;
        }
    ...
    public IServer getOrCreateServer(IProgressMonitor monitor) throws CoreException {
        ...
        if (useExistingServer.getSelection()) {
            ...
            @SuppressWarnings("unused") IRuntime //ServerCore.findRuntime("org.apache.sling.ide.launchpadRuntimeType");
            ...
            if (installToolingSupportBundle.getSelection()) {
                ...
                try {
                    ...
                    throw new CoreException(new Status(IStatus.ERROR, Activator.PLUGIN_ID, "Failed reading the tooling support bundle version", e));
                }
                ...
                if (installedVersion == null || ourVersion.compareTo(installedVersion) > 0) {
                    ...
                    try {
                        ...
                        throw new CoreException(new Status(IStatus.ERROR, Activator.PLUGIN_ID, "Failed installing the tooling support bundle version", e));
                    }
                }
            }
            ...
            try {
                ...
                if (finalVersion != null) {
                    wc.setAttribute(String.format(ISlingLaunchpadServer.PROP_BUNDLE_VERSION_FORMAT, EmbeddedArtifactLocator.SUPPORT_BUNDLE_SYMBOLIC_NAME), finalVersion.toString());
                }
