...
public class ScriptableNodeTest extends RepositoryScriptingTestBase {
    ...
    public void testPropertiesIterationNoWrapper() throws Exception {
        final String code = "var props = node.getProperties();" + " for(i in props) { out.print(props[i].name); out.print(' '); }";
    ...
    public void testAddNodeSpecificType() throws Exception {
        ...
        final String code = "var n = node.addNode('" + path + "', 'nt:folder');\n" + "out.print(n['jcr:primaryType']);\n";
    ...
    public void testGetSession() throws Exception {
        assertEquals("Root node found via node.session", "/", script.eval("node.session.getRootNode().getPath()", data));
        assertEquals("Root node found via node.getSession()", "/", script.eval("node.getSession().getRootNode().getPath()", data));
    }
    ...
    public void testMultiValReferencePropLookup() throws Exception {
        ...
        node.setProperty("multiRef", new Value[] { session.getValueFactory().createValue(refNode1), session.getValueFactory().createValue(refNode2) });
