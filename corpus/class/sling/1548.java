...
@Service
@Properties(value = { @Property(name = Constants.SERVICE_DESCRIPTION, value = "Apache Sling Tenant Provider") })
@Reference(name = "tenantSetup", referenceInterface = TenantCustomizer.class, cardinality = ReferenceCardinality.OPTIONAL_MULTIPLE, policy = ReferencePolicy.DYNAMIC)
public class TenantProviderImpl implements TenantProvider, TenantManager {
    ...
    private SortedMap<Comparable<Object>, TenantCustomizer> registeredTenantHandlers = new TreeMap<Comparable<Object>, TenantCustomizer>(Collections.reverseOrder());
    ...
    private void customizeTenant(final Resource tenantRes, final Tenant tenant) {
        ...
        if (tenantProps == null) {
            log.warn("create({}): Cannot get ModifiableValueMap for new tenant; will not store changed properties of TenantCustomizers", tenant.getId());
