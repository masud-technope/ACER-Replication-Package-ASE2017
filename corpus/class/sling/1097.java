...
@Properties({ @Property(name = Constants.SERVICE_DESCRIPTION, value = "Sling JCR Install Service"), @Property(name = Constants.SERVICE_VENDOR, value = "The Apache Software Foundation"), @Property(name = UpdateHandler.PROPERTY_SCHEMES, value = JcrInstaller.URL_SCHEME, unbounded = PropertyUnbounded.ARRAY), @Property(name = Constants.SERVICE_RANKING, intValue = 100) })
...
public class JcrInstaller implements UpdateHandler, ManagedService {
    ...
    static interface NodeConverter {
        ...
        InstallableResource convertNode(Node n, int priority) throws RepositoryException;
    }
        ...
        private volatile Session session;
         ...
         StoppableThread(final InstallerConfig cfg) throws RepositoryException {
            ...
            try {
                ...
                session.getWorkspace().getObservationManager().addEventListener(this, Event.NODE_ADDED | Event.NODE_REMOVED, "/", // isDeep
                false, null, null, // noLocal
                true);
                ...
                session.getWorkspace().getObservationManager().addEventListener(moveEventListener, Event.NODE_MOVED, "/", // isDeep
                true, null, null, // noLocal
                true);
    ...
    protected void activate(final ComponentContext context) {
        ...
        this.managedServiceRef = this.componentContext.getBundleContext().registerService(ManagedService.class.getName(), this, props);
    }
    ...
    private void findPathsToWatch(final InstallerConfig cfg, final Session session, final String rootPath) throws RepositoryException {
        Session s = null;
        try {
    ...
    public void runOneCycle(final InstallerConfig cfg, final Session session) {
        ...
        try {
            ...
            if (cfg.anyWatchFolderNeedsScan()) {
                ...
                if (scanningIsPaused(cfg, session)) {
                    if (!pauseMessageLogged) {
                        ...
                        logger.info("Detected signal for pausing the JCR Provider i.e. child nodes found under path {}. " + "JCR Provider scanning would not be performed", cfg.getPauseScanNodePath());
            ...
            for (final WatchedFolder wf : cfg.cloneWatchedFolders()) {
                ...
                if (toDo) {
                    installer.updateResources(URL_SCHEME, sr.toAdd.toArray(new InstallableResource[sr.toAdd.size()]), sr.toRemove.toArray(new String[sr.toRemove.size()]));
                }
            }
            ...
            if (scanWf || updateFoldersListTimer.expired()) {
                ...
                if (toRemove.size() > 0) {
                    ...
                    installer.updateResources(URL_SCHEME, null, toRemove.toArray(new String[toRemove.size()]));
                }
            }
        } catch (final Exception e) {
     ...
     */
    public UpdateResult handleUpdate(final String resourceType, final String id, final String url, final Dictionary<String, Object> dict, final Map<String, Object> attributes) {
     ...
     */
    public UpdateResult handleUpdate(final String resourceType, final String id, final String url, final InputStream is, final Map<String, Object> attributes) {
