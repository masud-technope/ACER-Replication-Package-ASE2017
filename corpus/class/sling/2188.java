...
public class AdvancedHttpDistributionTransport extends SimpleHttpDistributionTransport {
    ...
    public  AdvancedHttpDistributionTransport(DefaultDistributionLog log, boolean useCustomHeaders, String[] customHeaders, boolean useCustomBody, String customBody, DistributionEndpoint distributionEndpoint, DistributionPackageBuilder packageBuilder, DistributionTransportSecretProvider secretProvider, int maxNoOfPackages) {
    ...
    public void deliverPackage(@Nonnull ResourceResolver resourceResolver, @Nonnull DistributionPackage distributionPackage) throws DistributionException {
        log.info("delivering package {} to {} using auth {}", distributionPackage.getId(), distributionEndpoint.getUri(), secretProvider);
        try {
    ...
    private void deliverPackage(Executor executor, DistributionPackage distributionPackage, DistributionEndpoint distributionEndpoint) throws IOException {
        ...
        if (useCustomHeaders) {
            String[] customizedHeaders = getCustomizedHeaders(customHeaders, distributionPackage.getInfo().getRequestType().name(), distributionPackage.getInfo().getPaths());
        ...
        log.info("Distribution content of type {} for {} delivered: {}", new Object[] { type, Arrays.toString(distributionPackage.getInfo().getPaths()), content });
    }
