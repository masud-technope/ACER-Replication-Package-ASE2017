...
public class RequestHistoryConsolePlugin {
    ...
    public static void initPlugin(BundleContext context, int maxRequests, List<Pattern> storePatterns) {
        if (instance == null) {
            ...
            props.put(Constants.SERVICE_DESCRIPTION, "Web Console Plugin to display information about recent Sling requests");
            ...
            serviceRegistration = context.registerService("javax.servlet.Servlet", tmp, props);
            instance = tmp;
        }
    }
    ...
    public static final class Plugin extends HttpServlet {
        ...
        protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
            ...
            if (requests != null) {
                pw.println("<p class='statline ui-state-highlight'>Recorded " + requests.size() + " requests (max: " + requests.getMaxSize() + ")</p>");
            } else {
            ...
            if (info != null) {
                ...
                pw.println("<thead>");
                pw.println("<tr>");
                pw.printf("<th class='ui-widget-header'>Request %s (%s %s) by %s - RequestProgressTracker Info</th>%n", key, ResponseUtil.escapeXml(info.getMethod()), ResponseUtil.escapeXml(info.getPathInfo()), ResponseUtil.escapeXml(info.getUser()));
                pw.println("</tr>");
                pw.println("</thead>");
                pw.println("<tbody>");
