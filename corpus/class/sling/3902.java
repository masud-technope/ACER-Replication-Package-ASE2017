...
public class ScheduleInfoImpl implements ScheduleInfo, Serializable {
    ...
    private  ScheduleInfoImpl(final ScheduleType scheduleType, final int dayOfWeek, final int hourOfDay, final int minuteOfHour, final Date at, final int monthOfYear, final String expression) {
    ...
    public static ScheduleInfoImpl deserialize(final String s) {
        ...
        if (parts.length == 8 && parts[0].equals(VERSION)) {
            try {
                return new ScheduleInfoImpl(ScheduleType.valueOf(parts[1]), Integer.parseInt(parts[2]), Integer.parseInt(parts[3]), Integer.parseInt(parts[4]), (parts[5].equals("null") ? null : new Date(Long.parseLong(parts[5]))), Integer.parseInt(parts[6]), (parts[7].equals("null") ? null : parts[7]));
            } catch (final IllegalArgumentException iae) {
            }
        }
        return null;
    ...
    public int getDayOfMonth() {
        return (this.scheduleType == ScheduleType.MONTHLY || this.scheduleType == ScheduleType.YEARLY ? this.dayOfWeek : -1);
    }
    ...
    public String toString() {
        return "ScheduleInfo [scheduleType=" + scheduleType + ", dayOfWeek=" + dayOfWeek + ", hourOfDay=" + hourOfDay + ", minuteOfHour=" + minuteOfHour + ", at=" + at + ", monthOfYear=" + monthOfYear + ", expression=" + expression + "]";
    }
}
