...
public class BundleTestsProvider implements TestsProvider, BundleListener {
        ...
        for (Bundle b : bundleContext.getBundles()) {
            if (bundlesToUpdate.contains(b.getSymbolicName())) {
                ...
                if (testClasses != null) {
                    ...
                    log.debug("{} test classes found in bundle {}, added to our list", testClasses.size(), b.getSymbolicName());
                } else {
    ...
    public void bundleChanged(BundleEvent event) {
        ...
        if (getSlingTestRegexp(b) == null) {
            log.debug("Bundle {} does not have {} header, ignored", b.getSymbolicName(), SLING_TEST_REGEXP);
            return;
        }
        synchronized (changedBundles) {
