...
public class AccessManagerTestUtil extends AuthenticatedTestUtil {
    ...
    public String createTestFolder(String jsonContent) throws IOException {
        ...
        assertHttpStatus(location + DEFAULT_EXT, HttpServletResponse.SC_OK, "POST must redirect to created resource (" + location + ")");
        assertTrue("Node (" + location + ") must have generated name", !location.endsWith("/*"));
        assertTrue("Node (" + location + ") must created be under POST URL (" + testPath + ")", location.contains(testPath + "/"));
        return location;
    }
    ...
    public void assertPrivilege(Collection<String> privileges, boolean expected, String privilegeName) {
        if (expected != privileges.contains(privilegeName)) {
            fail("Expected privilege " + privilegeName + " to be " + (expected ? "included" : "NOT INCLUDED") + " in supplied list: " + privileges + ")");
        }
    }
}
