...
public class JcrItemResourceFactory {
    ...
    private static Item getSubitem(Node node, String relPath) throws RepositoryException {
        try {
            if (relPath.length() == 0) {
                // not using isEmpty() due to 1.5 compatibility
                return node;
            ...
            log.debug("getSubitem: Can't get subitem {} of {}: {}", new Object[] { relPath, node.toString(), e.toString() });
            return null;
        }
    }
    ...
    private boolean itemExists(final String path) {
        try {
            ...
            log.debug("itemExists: Error checking for existence of {}: {}", path, re.toString());
            return false;
        }
    }
}
