 ...
 */
@Component
@Property(name = Constants.SERVICE_DESCRIPTION, value = "Apache Sling Content Loader Implementation")
public class ContentLoaderService implements SynchronousBundleListener, BundleHelper {
        ...
        switch(event.getType()) {
            case BundleEvent.RESOLVED:
                ...
                try {
                    ...
                    log.error("bundleChanged: Problem loading initial content of bundle " + bundle.getSymbolicName() + " (" + bundle.getBundleId() + ")", t);
                } finally {
            ...
            case BundleEvent.UNINSTALLED:
                try {
                    ...
                    log.error("bundleChanged: Problem unloading initial content of bundle " + bundle.getSymbolicName() + " (" + bundle.getBundleId() + ")", t);
                } finally {
                    this.ungetSession(session);
                }
                break;
    ...
    public void createRepositoryPath(final Session writerSession, final String repositoryPath) throws RepositoryException {
    ...
    protected void activate(ComponentContext componentContext) {
        ...
        try {
            ...
            log.debug("Activated - attempting to load content from all " + "bundles which are neither INSTALLED nor UNINSTALLED");
            int ignored = 0;
            ...
            for (Bundle bundle : bundles) {
                if ((bundle.getState() & (Bundle.INSTALLED | Bundle.UNINSTALLED)) == 0) {
                    ...
                    try {
                        ...
                        log.error("Problem loading initial content of bundle " + bundle.getSymbolicName() + " (" + bundle.getBundleId() + ")", t);
                    } finally {
            ...
            log.debug("Out of {} bundles, {} were not in a suitable state for initial content loading", bundles.length, ignored);
        } catch (Throwable t) {
            log.error("activate: Problem while loading initial content and" + " registering mappings for existing bundles", t);
        } finally {
            this.ungetSession(session);
        }
    }
    ...
    public void unlockBundleContentInfo(final Session session, final Bundle bundle, final boolean contentLoaded, final List<String> createdNodes) throws RepositoryException {
        final String nodeName = bundle.getSymbolicName();
        final Node parentNode = (Node) session.getItem(BUNDLE_CONTENT_NODE);
        final Node bcNode = parentNode.getNode(nodeName);
        if (contentLoaded) {
