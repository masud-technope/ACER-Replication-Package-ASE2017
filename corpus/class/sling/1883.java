...
public class ScriptHelper implements SlingScriptHelper {
    ...
    public  ScriptHelper(final BundleContext ctx, final SlingScript script, final SlingHttpServletRequest request, final SlingHttpServletResponse response) {
        if (ctx == null) {
            throw new IllegalArgumentException("Bundle context must not be null.");
        }
        this.script = script;
        ...
        try {
            ...
            if (refs != null) {
                ...
                for (int i = 0; i < refs.length; i++) {
                    @SuppressWarnings("unchecked") final ServiceType service = (ServiceType) this.bundleContext.getService(refs[i]);
                    if (service != null) {
                        if (this.references == null) {
                            this.references = new ArrayList<ServiceReference>();
                        }
                ...
                if (objects.size() > 0) {
                    @SuppressWarnings("unchecked") ServiceType[] srv = (ServiceType[]) Array.newInstance(serviceType, objects.size());
