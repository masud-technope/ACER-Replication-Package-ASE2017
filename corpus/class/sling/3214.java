...
public class MultipleOsgiConsoleTest {
    ...
    static final String[] INSTANCE_NAMES = { "instance1", "instance2" };

    @Rule
    public final /** This causes our tests to be executed once for each specified instance */
    SlingInstancesRule rule = new SlingInstancesRule(INSTANCE_NAMES);
    ...
    public static void afterClass() {
        assertEquals("Expecting all instances to be tested", INSTANCE_NAMES.length * 2, numberOfTestsExecuted);
    }
    ...
    public void testSomeConsolePaths() throws Exception {
        ...
        final String[] subpaths = { "bundles", "components", "configMgr", "config", "licenses", "logs", "memoryusage", "services" };
        for (String subpath : subpaths) {
            final String path = "/system/console/" + subpath;
            instance.getRequestExecutor().execute(instance.getRequestBuilder().buildGetRequest(path).withCredentials(instance.getServerUsername(), instance.getServerPassword())).assertStatus(200);
        }
    }
