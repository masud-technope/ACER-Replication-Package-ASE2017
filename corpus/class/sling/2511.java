...
public class SlingFilterChainHelper {
    ...
    public synchronized Filter addFilter(final Filter filter, String pattern, final Long filterId, final int order, final String orderSource, FilterProcessorMBeanImpl mbean) {
        if (filterList == null) {
    ...
    public synchronized boolean removeFilterById(final Object filterId) {
        if (filterList != null) {
            for (Iterator<FilterHandle> fi = filterList.iterator(); fi.hasNext(); ) {
                ...
                if (test.getFilterId() == filterId || (test.getFilterId() != null && test.getFilterId().equals(filterId))) {
                    fi.remove();
                    filters = getFiltersInternal();
                    return true;
                }
