 ...
 * properties.
 */
@Component(immediate = true, metatype = true, label = "%jndi.name", description = "%jndi.description", name = "org.apache.sling.jcr.jackrabbit.server.JndiRegistrationSupport", policy = ConfigurationPolicy.REQUIRE)
...
public class JndiRegistrationSupport extends AbstractRegistrationSupport {
    ...
    protected boolean doActivate() {
        @SuppressWarnings("unchecked") Dictionary<String, Object> props = this.getComponentContext().getProperties();
        ...
        try {
            ...
            this.log(LogService.LOG_INFO, "Using JNDI context " + this.jndiContext.getEnvironment() + " to register repositories", null);
            return true;
        } catch (NamingException ne) {
            this.log(LogService.LOG_ERROR, "Problem setting up JNDI initial context, repositories will not be registered. Reason: " + ne.getMessage(), null);
        }
        // fallback to false
        return false;
    }
    ...
    protected Object bindRepository(String name, Repository repository) {
        if (this.jndiContext != null) {
            try {
                ...
                this.log(LogService.LOG_INFO, "Repository bound to JNDI as " + name, null);
                return repository;
            } catch (NamingException ne) {
    ...
    protected void unbindRepository(String name, Object data) {
        if (this.jndiContext != null) {
            try {
                ...
                this.log(LogService.LOG_INFO, "Repository " + name + " unbound from JNDI", null);
            } catch (NamingException ne) {
                this.log(LogService.LOG_ERROR, "Problem unregistering repository " + name, ne);
            }
        }
    }
}
