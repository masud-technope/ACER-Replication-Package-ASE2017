...
public class DefaultAuthenticationFeedbackHandler implements AuthenticationFeedbackHandler {
    ...
    public static boolean handleRedirect(final HttpServletRequest request, final HttpServletResponse response) {
        ...
        if (redirect != null) {
            ...
            try {
                ...
                LoggerFactory.getLogger(DefaultAuthenticationFeedbackHandler.class).error("handleRedirect: Failed to send redirect to " + redirect + ", aborting request without redirect", e);
            }
    ...
    private static String getValidatedRedirectTarget(final HttpServletRequest request) {
        ...
        if (!AuthUtil.isRedirectValid(request, redirect)) {
            LoggerFactory.getLogger(DefaultAuthenticationFeedbackHandler.class).error("handleRedirect: Redirect target '{}' is invalid, redirecting to '/'", redirect);
            redirect = "/";
        }
        return redirect;
    }
