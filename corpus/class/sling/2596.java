...
public class Result implements Iterable<ResultLog.Entry> {
    ...
    public enum Status implements  {
        ...
        // used by ResultLog for debug messages, not an actual output status
        DEBUG() {
        }
        , // used by ResultLog for info messages, not an actual output status
        INFO() {
        }
        , // no problem
        OK() {
        }
        , // health check detected something wrong but not critical
        WARN() {
        }
        , // health check detected a critical problem
        CRITICAL() {
        }
        , // health check did not execute properly
        HEALTH_CHECK_ERROR() {
        }
        ;
    }
