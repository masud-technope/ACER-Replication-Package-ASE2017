...
public class ValidationServiceImplTest {
    ...
    public void testValidateNeverCalledWithNullValues() throws Exception {
        Validator<String> myValidator = new Validator<String>() {
            ...
            @Override
            public String validate(@Nonnull String data, @Nonnull ValueMap valueMap, Resource resource, @Nonnull ValueMap arguments) throws SlingValidationException {
    ...
    public void testValueMapWithEmptyOptionalValue() throws Exception {
        ...
        hashMap.put("field1", "");
        ValidationResult vr = validationService.validate(new ValueMapDecorator(hashMap), vm);
        // check for correct error message Map<String, List<String>>
        Assert.assertFalse(vr.isValid());
        Assert.assertThat(vr.getFailureMessages(), Matchers.hasEntry("field1", Arrays.asList("Property does not match the pattern abc")));
    }
    ...
    public void testValueMapWithCorrectDataType() throws Exception {
        ...
        // check for correct error message Map<String, List<String>>
        Assert.assertFalse(vr.isValid());
        Assert.assertThat(vr.getFailureMessages(), Matchers.hasEntry("field2", Arrays.asList("Property does not match the pattern " + TEST_REGEX)));
    }
    ...
    public void testResourceWithMissingGrandChildProperty() throws Exception {
        ...
        Assert.assertThat(vr.getFailureMessages(), Matchers.hasEntry("child/grandchild/field1", Arrays.asList("Missing required property.")));
    ...
    public void testResourceWithValidatorLeveragingTheResource() throws Exception {
        Validator<String> extendedValidator = new Validator<String>() {
            ...
            @Override
            @CheckForNull
            public String validate(String data, ValueMap valueMap, Resource resource, ValueMap arguments) throws SlingValidationException {
    ...
    public void testResourceWithNestedChildrenAndPatternMatching() throws Exception {
        ...
        ValidationResult vr = validationService.validate(testResource, vm);
        Assert.assertFalse("resource should have been considered invalid", vr.isValid());
        Assert.assertThat(vr.getFailureMessages(), Matchers.hasEntry("child2/grandchild.*", Arrays.asList("Missing required child resource.")));
        Assert.assertThat(vr.getFailureMessages(), Matchers.hasEntry("child3/grandchild.*", Arrays.asList("Missing required child resource.")));
        Assert.assertThat(vr.getFailureMessages(), Matchers.hasEntry("child3/field1", Arrays.asList("Missing required property.")));
        Assert.assertThat(vr.getFailureMessages(), Matchers.hasEntry("siblingchild.*", Arrays.asList("Missing required child resource.")));
    ...
    public void testResourceWithPropertyPatternMatching() throws Exception {
        ...
        ValidationResult vr = validationService.validate(testResource, vm);
        Assert.assertFalse("resource should have been considered invalid", vr.isValid());
        Assert.assertThat(vr.getFailureMessages(), Matchers.hasEntry("field3", Arrays.asList("Property does not match the pattern \\d")));
        Assert.assertThat(vr.getFailureMessages(), Matchers.hasEntry("otherfield.*", Arrays.asList("Missing required property.")));
    ...
    public void testResourceWithMultivalueProperties() throws Exception {
        ...
        ValidationResult vr = validationService.validate(testResource, vm);
        Assert.assertFalse("resource should have been considered invalid", vr.isValid());
        Assert.assertThat(vr.getFailureMessages(), Matchers.hasEntry("field[1]", Arrays.asList("Property does not match the pattern \\d")));
        Assert.assertThat(vr.getFailureMessages().keySet(), Matchers.hasSize(1));
    }
    ...
    public void testValidateResourceRecursively() throws Exception {
        ...
        Assert.assertFalse("resource should have been considered invalid", vr.isValid());
        Assert.assertThat(vr.getFailureMessages(), Matchers.hasEntry("field1", Arrays.asList("Missing required property.")));
        Assert.assertThat(vr.getFailureMessages(), Matchers.hasEntry("child2/field2", Arrays.asList("Missing required property.")));
        Assert.assertThat(vr.getFailureMessages().keySet(), Matchers.hasSize(2));
    }
