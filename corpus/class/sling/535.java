...
public abstract class AbstractAuthorizablePostServlet extends AbstractPostServlet {
     ...
     */
    protected Map<String, RequestProperty> collectContent(Map<String, ?> properties, String authorizablePath) {
     ...
     *             removing properties.
     */
    protected void processDeletes(Authorizable authorizable, Map<String, RequestProperty> reqProperties, List<Modification> changes) throws RepositoryException {
     ...
     * @throws ServletException if an internal error occurs
     */
    protected void writeContent(Session session, Authorizable authorizable, Map<String, RequestProperty> reqProperties, List<Modification> changes) throws RepositoryException {
        for (RequestProperty prop : reqProperties.values()) {
            if (prop.hasValues()) {
                ...
                if (authorizable.isGroup()) {
                    ...
                    if (prop.getName().equals("userId") || prop.getName().equals("pwd") || prop.getName().equals("pwdConfirm")) {
                        // skip these
                        continue;
                    }
                }
     ...
     * @throws RepositoryException if a repository error occurs.
     */
    private boolean removePropertyIfExists(Authorizable authorizable, String name) throws RepositoryException {
    ...
    protected boolean hasItemPathPrefix(String name) {
        return name.startsWith(SlingPostConstants.ITEM_PREFIX_ABSOLUTE) || name.startsWith(SlingPostConstants.ITEM_PREFIX_RELATIVE_CURRENT) || name.startsWith(SlingPostConstants.ITEM_PREFIX_RELATIVE_PARENT);
    }
