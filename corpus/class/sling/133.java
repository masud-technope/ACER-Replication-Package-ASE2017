...
@Properties({ @Property(name = Constants.SERVICE_VENDOR, value = "The Apache Software Foundation"), @Property(name = Constants.SERVICE_DESCRIPTION, value = "Sling Servlet") })
@References({ @Reference(name = "ErrorHandler", referenceInterface = ErrorHandler.class, cardinality = ReferenceCardinality.OPTIONAL_UNARY, policy = ReferencePolicy.DYNAMIC, bind = "setErrorHandler", unbind = "unsetErrorHandler"), @Reference(name = "ServletResolver", referenceInterface = ServletResolver.class, cardinality = ReferenceCardinality.OPTIONAL_UNARY, policy = ReferencePolicy.DYNAMIC, bind = "setServletResolver", unbind = "unsetServletResolver"), @Reference(name = "MimeTypeService", referenceInterface = MimeTypeService.class, cardinality = ReferenceCardinality.OPTIONAL_UNARY, policy = ReferencePolicy.DYNAMIC, bind = "setMimeTypeService", unbind = "unsetMimeTypeService"), @Reference(name = "AuthenticationSupport", referenceInterface = AuthenticationSupport.class, cardinality = ReferenceCardinality.OPTIONAL_UNARY, policy = ReferencePolicy.DYNAMIC, bind = "setAuthenticationSupport", unbind = "unsetAuthenticationSupport") })
public class SlingMainServlet extends GenericServlet {
    ...
    @Property(value = { "X-Content-Type-Options=nosniff" }, label = "Additional response headers", description = "Provides mappings for additional response headers " + "Each entry is of the form 'bundleId [ \":\" responseHeaderName ] \"=\" responseHeaderValue' ", unbounded = PropertyUnbounded.ARRAY)
    ...
    public void service(ServletRequest req, ServletResponse res) throws ServletException {
        if (req instanceof HttpServletRequest && res instanceof HttpServletResponse) {
            ...
            try {
                ...
                requestProcessor.doProcessRequest(request, (HttpServletResponse) res, resolver);
    ...
    protected void activate(final BundleContext bundleContext, final Map<String, Object> componentConfig) {
        ...
        final Hashtable<String, Object> configuration = new Hashtable<String, Object>(componentConfig);
        ...
        allowTrace = PropertiesUtil.toBoolean(componentConfig.get(PROP_ALLOW_TRACE), DEFAULT_ALLOW_TRACE);
        // configure the request limits
        RequestData.setMaxIncludeCounter(PropertiesUtil.toInteger(componentConfig.get(PROP_MAX_INCLUSION_COUNTER), RequestData.DEFAULT_MAX_INCLUSION_COUNTER));
        RequestData.setMaxCallCounter(PropertiesUtil.toInteger(componentConfig.get(PROP_MAX_CALL_COUNTER), RequestData.DEFAULT_MAX_CALL_COUNTER));
        ...
        if (componentConfig.get(PROP_DEFAULT_PARAMETER_ENCODING) != null) {
            log.warn("Configure default request parameter encoding with 'org.apache.sling.parameters.config' configuration; the property " + PROP_DEFAULT_PARAMETER_ENCODING + "=" + componentConfig.get(PROP_DEFAULT_PARAMETER_ENCODING) + " is ignored");
        }
        // register the servlet and resources
        try {
            Dictionary<String, String> servletConfig = toStringConfig(configuration);
            this.httpService.registerServlet(SLING_ROOT, this, servletConfig, slingHttpContext);
        ...
        filterManager = new ServletFilterManager(bundleContext, slingServletContext, PropertiesUtil.toBoolean(componentConfig.get(PROP_FILTER_COMPAT_MODE), DEFAULT_FILTER_COMPAT_MODE));
        filterManager.open();
        ...
        try {
            int maxRequests = PropertiesUtil.toInteger(componentConfig.get(PROP_MAX_RECORD_REQUESTS), RequestHistoryConsolePlugin.STORED_REQUESTS_COUNT);
        ...
        requestProcessorRegistration = bundleContext.registerService(SlingRequestProcessor.NAME, requestProcessor, srpProps);
    }
    ...
    protected void deactivate() {
        ...
        try {
            ...
            log.debug("Problem unregistering web console request recorder plugin.", t);
        }
    ...
    public void unsetAuthenticationSupport(final AuthenticationSupport authenticationSupport) {
