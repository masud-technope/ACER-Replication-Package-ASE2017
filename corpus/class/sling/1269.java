...
public class ScanningLoopTest extends RepositoryTestBase {
    ...
    public void testDeleteFile() throws Exception {
        ...
        eventHelper.waitForEvents(TIMEOUT);
        Thread.sleep(JcrInstaller.RUN_LOOP_DELAY_MSEC * 2);
        assertEvents("Expected at least one folder scan event", sf, JcrInstaller.SCAN_FOLDERS_COUNTER);
        assertIdle();
    }
    ...
    public void testDeleteLibsFolder() throws Exception {
        ...
        eventHelper.waitForEvents(TIMEOUT);
        Thread.sleep(JcrInstaller.RUN_LOOP_DELAY_MSEC * 2);
        assertEvents("Expected at least one folders list update event", uc, JcrInstaller.UPDATE_FOLDERS_LIST_COUNTER);
        assertIdle();
    }
