...
public class SimpleConstraintChecker {
    ...
    public void check(Object inputValue, String constraint, ResultLog resultLog) {
        ...
        try {
            ...
            resultLog.add(new ResultLog.Entry(Result.Status.WARN, "Invalid numeric value [" + inputValue + "] while evaluating " + constraint));
        }
        if (matches) {
            resultLog.add(new ResultLog.Entry(Result.Status.DEBUG, "Value [" + inputValue + "] matches constraint [" + constraint + "]"));
        } else {
            resultLog.add(new ResultLog.Entry(Result.Status.WARN, "Value [" + inputValue + "] does not match constraint [" + constraint + "]"));
        }
    }
}
