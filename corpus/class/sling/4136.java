...
class JcrNodeResource extends // this should be package private, see SLING-1414
JcrItemResource<Node> {
     ...
     */
    public  JcrNodeResource(final ResourceResolver resourceResolver, final String path, final String version, final Node node, final HelperData helper) {
    ...
    public <Type> Type adaptTo(Class<Type> type) {
        if (type == Node.class || type == Item.class) {
            ...
            try {
                ...
                LOGGER.debug("adaptTo(PersistableValueMap): Cannot set properties on {}", this);
            } catch (RepositoryException e) {
                LOGGER.debug("adaptTo(PersistableValueMap): Unexpected problem for {}", this);
            }
    ...
    Iterator<Resource> listJcrChildren() {
        try {
            if (getNode().hasNodes()) {
                return new JcrNodeResourceIterator(getResourceResolver(), path, version, getNode().getNodes(), this.helper, null);
            }
