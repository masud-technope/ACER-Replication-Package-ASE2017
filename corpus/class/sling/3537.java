...
public abstract class AbstractSlingRepository extends AbstractNamespaceMappingRepository implements SlingRepository, Runnable {
    ...
    private void setDefaultWorkspace(String defaultWorkspace) {
        ...
        log(LogService.LOG_DEBUG, "setDefaultWorkspace: Setting the default workspace to " + defaultWorkspace);
    ...
    public final Session loginAdministrative(String workspace) throws RepositoryException {
        ...
        log(LogService.LOG_WARNING, "SlingRepository.loginAdministrative is deprecated. Please use SlingRepository.loginService.");
    ...
    public final Session loginService(String subServiceName, String workspace) throws LoginException, RepositoryException {
        log(LogService.LOG_ERROR, "loginService: Cannot get using Bundle because this SlingRepository service is not a ServiceFactory");
        throw new LoginException();
    }
    ...
    public Session login(Credentials credentials, String workspace) throws LoginException, NoSuchWorkspaceException, RepositoryException {
        ...
        try {
            log(LogService.LOG_DEBUG, "login: Logging in to workspace '" + workspace + "'");
            ...
            if (workspace == null) {
                ...
                log(LogService.LOG_DEBUG, "login: Using " + defaultWorkspace + " as the default workspace instead of 'null'");
     ...
     * @since 2.2 (bundle version 2.2.0)
     */
    final Session loginService(final Bundle usingBundle, final String subServiceName, final String workspace) throws LoginException, RepositoryException {
    ...
    protected Repository acquireRepository() {
        ...
        // are ignored
        String overrideUrl = (String) componentContext.getProperties().get(RepositoryAccessor.REPOSITORY_URL_OVERRIDE_PROPERTY);
        if (overrideUrl == null) {
            overrideUrl = componentContext.getBundleContext().getProperty(RepositoryAccessor.REPOSITORY_URL_OVERRIDE_PROPERTY);
        }
        if (overrideUrl != null && overrideUrl.length() > 0) {
            log(LogService.LOG_INFO, "acquireRepository: Will not use embedded repository due to property " + RepositoryAccessor.REPOSITORY_URL_OVERRIDE_PROPERTY + "=" + overrideUrl + ", acquiring repository using that URL");
            return getRepositoryAccessor().getRepositoryFromURL(overrideUrl);
        }
        log(LogService.LOG_DEBUG, "acquireRepository: No existing repository to access");
        return null;
    }
    ...
    protected boolean pingRepository(Repository repository) {
        if (repository != null) {
            try {
                ...
                log(LogService.LOG_DEBUG, "pingRepository: Repository " + repository + " does not seem to be available any more", t);
            }
        }
    ...
    protected void activate(final ComponentContext componentContext) throws Exception {
        ...
        setDefaultWorkspace(this.getProperty(properties, PROPERTY_DEFAULT_WORKSPACE, null));
        this.anonUser = this.getProperty(properties, PROPERTY_ANONYMOUS_USER, DEFAULT_ANONYMOUS_USER);
        this.anonPass = this.getProperty(properties, PROPERTY_ANONYMOUS_PASS, DEFAULT_ANONYMOUS_PASS).toCharArray();
        this.adminUser = this.getProperty(properties, PROPERTY_ADMIN_USER, DEFAULT_ADMIN_USER);
        this.adminPass = this.getProperty(properties, PROPERTY_ADMIN_PASS, DEFAULT_ADMIN_PASS).toCharArray();
        this.disableLoginAdministrative = !this.getProperty(properties, PROPERTY_LOGIN_ADMIN_ENABLED, DEFAULT_LOGIN_ADMIN_ENABLED);
        ...
        try {
            if (startRepository()) {
                ...
                log(LogService.LOG_WARNING, "Repository startup failed, will try later");
            }
        } catch (Throwable t) {
            log(LogService.LOG_WARNING, "activate: Unexpected problem starting repository", t);
        }
    ...
    protected void deactivate(final ComponentContext componentContext) {
        ...
        if (repository != null || repositoryService != null) {
            log(LogService.LOG_INFO, "deactivate: Repository still running, forcing shutdown");
            try {
                stopRepository();
            } catch (Throwable t) {
                log(LogService.LOG_WARNING, "deactivate: Unexpected problem stopping repository", t);
            }
        }
    ...
    private boolean createWorkspace(String workspace) {
        this.log(LogService.LOG_INFO, "createWorkspace: Requested workspace " + workspace + " does not exist, trying to create");
        ...
        try {
            ...
            this.log(LogService.LOG_ERROR, "createWorkspace: Cannot create requested workspace " + workspace, t);
        } finally {
            if (tmpSession != null) {
                tmpSession.logout();
            }
    ...
    private void stopRepositoryPinger() {
        ...
        if (rpThread.isAlive()) {
            log(LogService.LOG_ERROR, "stopRepositoryPinger: Timed waiting for thread " + rpThread + " to terminate");
        }
    }
        ...
        try {
            ...
            if (newRepo != null) {
                ...
                if (pingRepository(newRepo)) {
                    repository = newRepo;
                    if (pingAndCheck()) {
                        log(LogService.LOG_DEBUG, "startRepository: pingRepository() and pingAndCheck() successful, calling setupRepository()");
                    ...
                    // scratch later
                    log(LogService.LOG_DEBUG, "pingRepository() successful but pingAndCheck() fails, calling disposeRepository()");
                    // drop reference
                    repository = null;
                } else {
            ...
            log(LogService.LOG_ERROR, "startRepository: Uncaught Throwable trying to access Repository, calling stopRepository()", t);
            stopRepository();
        }
        return false;
    }
    ...
    private void stopRepository() {
        if (repositoryService != null) {
            try {
                ...
                log(LogService.LOG_INFO, "stopRepository: Uncaught problem unregistering the repository service", t);
            }
        ...
        if (repository != null) {
            ...
            try {
                ...
                log(LogService.LOG_INFO, "stopRepository: Uncaught problem tearing down the repository", t);
            }
            try {
                disposeRepository(oldRepo);
            } catch (Throwable t) {
                log(LogService.LOG_INFO, "stopRepository: Uncaught problem disposing the repository", t);
            }
        }
    }
            ...
            while (running) {
                ...
                if (running) {
                    ...
                    if (repo == null) {
                        ...
                        log(LogService.LOG_INFO, "run: Repository not accessible anymore, unregistering service");
                        stopRepository();
                        newPollTime = pollTimeInActiveSeconds * MSEC;
                    }
                    if (newPollTime != pollTimeMsec) {
                        pollTimeMsec = newPollTime;
                        log(LogService.LOG_DEBUG, "Repository Pinger interval set to " + pollTimeMsec + " msec, repository is " + (ok ? "available" : "NOT available"));
                    }
                }
            }
    ...
    private static class SlingRepositoryProxyHandler implements InvocationHandler {
         ...
         */
        static Object createProxy(final String[] interfaceNames, final AbstractSlingRepository delegatee, final Bundle usingBundle) {
            ...
            for (int i = 0; i < interfaces.length; i++) {
                try {
                    ...
                    LoggerFactory.getLogger(SlingRepositoryProxyHandler.class).error("createProxy: Cannot load interface class " + interfaceNames[i], e);
                    return null;
                }
            }
            // create the proxy
