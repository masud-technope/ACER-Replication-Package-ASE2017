 ...
 */
@Component(immediate = true, metatype = true, label = "%rmi.name", description = "%rmi.description", name = "org.apache.sling.jcr.jackrabbit.server.RmiRegistrationSupport", policy = ConfigurationPolicy.REQUIRE)
@org.apache.felix.scr.annotations.Properties({ @Property(name = "service.vendor", value = "The Apache Software Foundation", propertyPrivate = true), @Property(name = "service.description", value = "RMI based Repository Registration", propertyPrivate = true) })
@References({ @Reference(name = "Repository", policy = ReferencePolicy.DYNAMIC, cardinality = ReferenceCardinality.OPTIONAL_MULTIPLE, referenceInterface = Repository.class), @Reference(referenceInterface = LogService.class, bind = "bindLog", unbind = "unbindLog", cardinality = ReferenceCardinality.OPTIONAL_UNARY, policy = ReferencePolicy.DYNAMIC) })
    ...
    protected void doDeactivate() {
        ...
        if (this.registry != null && this.registryIsPrivate) {
            try {
                ...
                this.log(LogService.LOG_INFO, "Unexported private RMI Registry at " + this.registryPort, null);
            } catch (NoSuchObjectException nsoe) {
                this.log(LogService.LOG_INFO, "Cannot unexport private RMI Registry reference", nsoe);
            }
        }
        this.registry = null;
    }
    ...
    private Registry getPrivateRegistry() {
        if (this.registry == null) {
            try {
                ...
                this.log(LogService.LOG_INFO, "Using private RMI Registry at " + this.registryPort, null);
            } catch (RemoteException re) {
                this.log(LogService.LOG_INFO, "Cannot create private registry, trying existing registry at " + this.registryPort + ", reason: " + re.toString(), null);
                try {
                    this.registry = LocateRegistry.getRegistry(this.registryPort);
                    this.registryIsPrivate = false;
                    this.log(LogService.LOG_INFO, "Trying existing registry at " + this.registryPort, null);
                } catch (RemoteException pre) {
                    this.log(LogService.LOG_ERROR, "Cannot get existing registry, will not register repositories on RMI", pre);
                }
            }
        }
    ...
    private class RmiRegistration {
        ...
        private void register(String rmiName, Repository repository) {
            ...
            try {
                ...
                RmiRegistrationSupport.this.log(LogService.LOG_WARNING, "Cannot contact RMI registry at " + RmiRegistrationSupport.this.registryPort + ", repository not registered", null);
            } catch (Exception e) {
