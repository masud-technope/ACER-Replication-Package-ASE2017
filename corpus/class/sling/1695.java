...
public class OsgiInstallerImpl implements OsgiInstaller, ResourceChangeListener, RetryHandler, InfoProvider, Runnable {
    ...
    private boolean hasBundleUpdateTask(final SortedSet<InstallTask> tasks) {
        ...
        for (final InstallTask task : tasks) {
            if (task.isAsynchronousTask()) {
                // async task is executed immediately#
                result = false;
                break;
     ...
     */
    public void resourceAddedOrUpdated(final String resourceType, final String entityId, final InputStream is, final Dictionary<String, Object> dict, final Map<String, Object> attributes) {
        try {
                ...
                if (!this.persistentList.isSpecialEntityId(entityId)) {
                    ...
                    for (final TaskResource tr : group.getResources()) {
                        resources.add(new Resource() {
                            ...
                            @Override
                            public String toString() {
                                return "resource[entityId=" + this.getEntityId() + ", scheme=" + this.getScheme() + ", url=" + this.getURL() + ", type=" + this.getType() + ", state=" + this.getState() + ", version=" + this.getVersion() + ", lastChange=" + this.getLastChange() + ", priority=" + this.getPriority() + ", digest=" + this.getDigest() + "]";
                            }
                        });
                    }
