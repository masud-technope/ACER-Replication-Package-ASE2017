...
public class CodeGenVisitor implements CommandVisitor {
    ...
    private void initVariable(VariableDescriptor descriptor, JavaSource initSource) {
        ...
        if (listCoercionVar != null) {
            ...
            initSource.beginAssignment(listCoercionVar, SourceGenConstants.COLLECTION_TYPE).nullLiteral().endStatement();
        }
    }
    ...
    public void visit(Conditional.Start conditional) {
        ...
        if (descriptor.getType() == Type.BOOLEAN) {
            ...
            source.startMethodCall(SourceGenConstants.RENDER_UTILS, RenderUtils.BOOLEAN_COERCE).append(descriptor.getAssignedName()).endCall();
        }
        source.completeIf();
    }
    ...
    public void visit(VariableBinding.Start variableBinding) {
        ...
        ExpressionTranslator.buildExpression(variableBinding.getExpression(), source, analyzer, typeInfo);
        source.endStatement();
    }
    ...
    public void visit(VariableBinding.Global globalAssignment) {
        ...
        ExpressionTranslator.buildExpression(globalAssignment.getExpression(), source, analyzer, typeInfo);
        source.endStatement();
        String listCoercionVar = descriptor.getListCoercion();
        if (listCoercionVar != null) {
    ...
    public void visit(OutVariable outVariable) {
        ...
        source.startStatement().startMethodCall(SourceGenConstants.OUT_BUFFER, SourceGenConstants.WRITE_METHOD).startMethodCall(SourceGenConstants.RENDER_UTILS, RenderUtils.STRING_COERCE).append(variable).endCall().endCall().endStatement();
    }

    @Override
    public void visit(OutText outText) {
        source.startStatement().startMethodCall(SourceGenConstants.OUT_BUFFER, SourceGenConstants.WRITE_METHOD).stringLiteral(outText.getText()).endCall().endStatement();
    }
    ...
    public void visit(Loop.Start loop) {
        ...
        source.beginIf().append(collectionVar).equality().nullLiteral().completeIf().startStatement().append(collectionVar).assign().startMethodCall(SourceGenConstants.RENDER_UTILS, RenderUtils.COLLECTION_COERCE).append(listVariable).endCall().endStatement().endIf();
    ...
    public void visit(Procedure.Call procedureCall) {
        ...
        source.startStatement().startCall(SourceGenConstants.CALL_UNIT_METHOD, false).append(SourceGenConstants.RENDER_CONTEXT_INSTANCE).separateArgument().append(templateVar).separateArgument().append(argVar).endCall().endStatement();
    }
