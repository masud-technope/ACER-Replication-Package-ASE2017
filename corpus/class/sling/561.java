...
public class SlingWebConsoleSecurityProvider2 extends AbstractWebConsoleSecurityProvider implements WebConsoleSecurityProvider2 {
    ...
    public boolean authenticate(final HttpServletRequest request, final HttpServletResponse response) {
        if (this.authentiationSupport.handleSecurity(request, response)) {
            ...
            final ResourceResolver resolver = (resolverObject instanceof ResourceResolver) ? (ResourceResolver) resolverObject : null;
            if (resolver != null) {
                ...
                if (session != null) {
                    try {
                        ...
                        logger.info("authenticate: Generic problem trying grant User " + " access to the Web Console", re);
                    }
                }
            }
    ...
    private User authenticate(final Session session) throws RepositoryException {
        ...
        if (session instanceof JackrabbitSession) {
            ...
            if (a instanceof User) {
                ...
                // check groups
                @SuppressWarnings("unchecked") Iterator<Group> gi = a.memberOf();
                while (gi.hasNext()) {
            ...
            logger.info("authenticate: Jackrabbit Session required to grant access to the Web Console for {}; got {}", userId, session.getClass());
        }
        return null;
    }
