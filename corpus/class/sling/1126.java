...
@Properties({ @Property(name = "felix.webconsole.label", value = "slingevent"), @Property(name = "felix.webconsole.title", value = "Jobs"), @Property(name = "felix.webconsole.category", value = "Sling"), @Property(name = JobConsumer.PROPERTY_TOPICS, value = { "sling/webconsole/test" }) })
public class WebConsolePlugin extends HttpServlet implements JobConsumer {
    ...
    private void printQueueConfiguration(final HttpServletRequest req, final PrintWriter pw, final InternalQueueConfiguration c) {
        pw.println("<div class='ui-widget-header ui-corner-top buttonGroup'>");
        pw.printf("<span style='float: left; margin-left: 1em'>Job Queue Configuration: %s</span>%n", ResponseUtil.escapeXml(c.getName()));
    ...
    private void printForm(final PrintWriter pw, final String qeueName, final String buttonLabel, final String cmd) {
        pw.printf("<button class='ui-state-default ui-corner-all' onclick='javascript:eventingsubmit(\"%s\", \"%s\");'>" + "%s</button>", ResponseUtil.escapeXml(cmd), (qeueName != null ? ResponseUtil.escapeXml(qeueName) : ""), ResponseUtil.escapeXml(buttonLabel));
    }
