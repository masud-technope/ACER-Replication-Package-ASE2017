...
public class MockBundleContextDynamicReferncesOsgiR6Test {
    ...
    public void testReferenceWithTargetFilter() {
        ...
        bundleContext.registerService(ServiceInterface3.class.getName(), dependency3a, MapUtil.toDictionary(ImmutableMap.<String, Object>of("prop1", "abc")));
        bundleContext.registerService(ServiceInterface3.class.getName(), dependency3b, MapUtil.toDictionary(ImmutableMap.<String, Object>of("prop1", "def")));
    ...
    private void assertDependencies2(ServiceInterface2... instances) {
        assertEquals(ImmutableSet.<ServiceInterface2>copyOf(instances), ImmutableSet.<ServiceInterface2>copyOf(service.getReferences2()));
    }

    private void assertDependencies3(ServiceSuperInterface3... instances) {
        assertEquals(ImmutableSet.<ServiceSuperInterface3>copyOf(instances), ImmutableSet.<ServiceSuperInterface3>copyOf(service.getReferences3()));
    }

    private void assertDependencies3Filtered(ServiceSuperInterface3... instances) {
        assertEquals(ImmutableSet.<ServiceSuperInterface3>copyOf(instances), ImmutableSet.<ServiceSuperInterface3>copyOf(service.getReferences3Filtered()));
    }
}
