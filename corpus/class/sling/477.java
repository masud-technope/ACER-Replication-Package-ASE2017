...
public class SlingLaunchpadServer extends ServerDelegate implements ISlingLaunchpadServer {
    ...
    public IStatus canModifyModules(IModule[] toAdd, IModule[] toRemove) {
        ...
        for (IModule module : toAdd) {
            if (!MODULE_TYPE_SLING_CONTENT.equals(module.getModuleType().getId()) && !MODULE_TYPE_SLING_BUNDLE.equals(module.getModuleType().getId())) {
                return new Status(IStatus.ERROR, Activator.PLUGIN_ID, "Will only handle modules of type 'sling.content' or 'sling.bundle'");
            }
        }
        return Status.OK_STATUS;
    }
    ...
    public IModule[] getRootModules(IModule arg0) throws CoreException {
        ...
        throw new CoreException(new Status(IStatus.ERROR, "org.apache.sling.slingclipse", 0, "Will only handle modules of type 'sling.content' or 'sling.bundle'", null));
    }
