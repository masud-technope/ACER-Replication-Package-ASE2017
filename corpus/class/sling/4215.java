...
public class VotingHandler implements EventHandler {
    ...
    public static enum VotingDetail implements  {
    ...
    protected void activate(final ComponentContext context) {
        ...
        logger = LoggerFactory.getLogger(this.getClass().getCanonicalName() + "." + slingId);
    ...
    public synchronized Map<VotingView, VotingDetail> analyzeVotings(final ResourceResolver resourceResolver) throws PersistenceException {
        ...
        VotingView winningVote = VotingHelper.getWinningVoting(resourceResolver, config);
        if (winningVote != null) {
            if (winningVote.isInitiatedBy(slingId)) {
                logger.info("analyzeVotings: my voting was winning. I'll mark it as established then! " + winningVote);
                try {
                ...
                logger.info("analyzeVotings: there is a winning vote. No need to vote any further. Expecting it to get promoted to established: " + winningVote);
        ...
        for (VotingView voting : ongoingVotings) {
            ...
            if (liveComparison != null) {
                if (!votedNo) {
                    logger.info("analyzeVotings: vote doesnt match my live view, voting no. " + "comparison result: " + liveComparison + ", vote: " + voting);
                    voting.vote(slingId, false, null);
                    result.put(voting, VotingDetail.VOTED_NO);
                } else {
                    result.put(voting, VotingDetail.UNCHANGED);
            ...
            if (!votedYes) {
                logger.info("analyzeVotings: not timed out, no no-votes, matches live, still ongoing, " + "I have not yet voted yes, so noting candidate for yes as: " + voting);
            }
            yesVote = voting;
        }
        if (yesVote != null) {
    ...
    public void cleanupTimedoutVotings(final ResourceResolver resourceResolver) {
        List<VotingView> timedoutVotings = VotingHelper.listTimedoutVotings(resourceResolver, config);
