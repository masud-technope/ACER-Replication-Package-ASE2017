...
public class ResourceCollector extends AbstractResourceCollector {
    ...
    public static ResourceCollector create(final Resource resource, final String extension, final String[] executionPaths, final String[] defaultExtensions, final String methodName, final String[] selectors) {
     ...
     */
    public  ResourceCollector(final String methodName, final String baseResourceType, final Resource resource, final String[] executionPaths) {
        super((baseResourceType != null ? baseResourceType : ServletResolverConstants.DEFAULT_SERVLET_NAME), resource.getResourceType(), resource.getResourceSuperType(), null, executionPaths);
        ...
        final String key = methodName + ':' + baseResourceType + ':' + extension + "::" + (this.resourceType == null ? "" : this.resourceType) + ':' + (this.resourceSuperType == null ? "" : this.resourceSuperType);
     ...
     *            is assumed.
     */
    private  ResourceCollector(final Resource resource, final String extension, final String[] executionPaths, final boolean isDefaultExtension, final String methodName, final String[] selectors) {
        super(ServletResolverConstants.DEFAULT_SERVLET_NAME, resource.getResourceType(), resource.getResourceSuperType(), extension, executionPaths);
        this.methodName = methodName;
        this.suffExt = "." + extension;
        this.suffMethod = "." + methodName;
        this.suffExtMethod = suffExt + suffMethod;
        ...
        // create the hash code once
        final String key = methodName + ':' + baseResourceType + ':' + extension + ':' + StringUtils.join(requestSelectors, '.') + ':' + (this.resourceType == null ? "" : this.resourceType) + ':' + (this.resourceSuperType == null ? "" : this.resourceSuperType);
        this.hashCode = key.hashCode();
    }

    @Override
    protected void getWeightedResources(final Set<Resource> resources, final Resource location) {
        ...
        do {
            ...
            while (children.hasNext()) {
                ...
                if (isGet && checkScriptName(scriptName, selector, parentName, suffExt, null, resources, child, selIdx)) {
                    continue;
                }
                if (checkScriptName(scriptName, selector, parentName, suffExtMethod, suffMethod, resources, child, selIdx)) {
                    continue;
                }
                // compatibility.
     ...
     */
    private boolean checkScriptName(final String scriptName, final String selector, final String parentName, final String suffix, final String htmlSuffix, final Set<Resource> resources, final Resource child, final int selIdx) {
        ...
        if (matches(scriptName, parentName, suffix)) {
            addWeightedResource(resources, child, selIdx, WeightedResource.WEIGHT_EXTENSION + WeightedResource.WEIGHT_PREFIX);
            return true;
        }
        if (scriptName.equals(suffix.substring(1))) {
            addWeightedResource(resources, child, selIdx, WeightedResource.WEIGHT_EXTENSION);
            return true;
        }
    ...
    private void addLocationServlet(final Set<Resource> resources, final Resource location) {
    ...
    public boolean equals(Object obj) {
        ...
        if (super.equals(obj)) {
            ...
            if (isGet == o.isGet && isDefaultExtension == o.isDefaultExtension && numRequestSelectors == o.numRequestSelectors && stringEquals(methodName, o.methodName)) {
