...
public class PersistentResourceList {
    ...
    public  PersistentResourceList(final File dataFile, final InstallationListener listener) {
        ...
        if (this.getEntityResourceList(RESTART_ACTIVE_BUNDLES_ENTITY_ID) == null) {
            final RegisteredResource rr = this.addOrUpdate(new InternalResource("$sling-installer$", RESTART_ACTIVE_BUNDLES_ID, null, new Hashtable<String, Object>(), InstallableResource.TYPE_PROPERTIES, "1", null, null, null));
    ...
    private void checkInstallable(final RegisteredResourceImpl input) {
        if (!InstallableResource.TYPE_FILE.equals(input.getType()) && !InstallableResource.TYPE_PROPERTIES.equals(input.getType())) {
        ...
        try {
            for (int i = 0; i < result.length; i++) {
                ...
                if (tr.getResourceType() != null && tr.getId() == null) {
                    logger.error("Result for {} contains new resource type {} but no unique id!", resource, tr.getResourceType());
                    continue;
                }
