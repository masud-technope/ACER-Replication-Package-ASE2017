...
public class FindPeopleQuery implements Query {
     ...
     */
    public  FindPeopleQuery(int searchType, String searchQuery, boolean isPrefixQuery, long offset, long maxResults) {
    ...
    public <T> void build(QueryBuilder<T> builder) {
        ...
        //max + 1 so we can tell if there are more on the next page
        builder.setLimit(offset, maxResults + 1);
        ...
        if (isPrefixQuery) {
            ...
            builder.setCondition(builder.or(builder.nameMatches(encodedTerm + "%"), builder.nameMatches(lcEncodedTerm + "%")));
        } else {
            //check for a match in the name or displayName
            builder.setCondition(builder.or(builder.nameMatches("%" + encodedTerm + "%"), builder.like("@displayName", "%" + encodedTerm + "%")));
        }
    }
