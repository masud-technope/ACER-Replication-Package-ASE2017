...
@Properties({ @Property(name = "service.description", value = "Default GET Servlet"), @Property(name = "service.vendor", value = "The Apache Software Foundation"), // Use this as a default servlet for Sling
@Property(name = "sling.servlet.resourceTypes", value = "sling/servlet/default", propertyPrivate = true), @Property(name = "sling.servlet.prefix", intValue = -1, propertyPrivate = true), // Generic handler for all get requests
...
public class DefaultGetServlet extends SlingSafeMethodsServlet {
    ...
    protected void activate(ComponentContext ctx) {
        ...
        this.index = OsgiUtil.toBoolean(props.get(INDEX_PROPERTY), DEFAULT_INDEX_PROPERTY);
        this.indexFiles = OsgiUtil.toStringArray(props.get(INDEX_FILES_PROPERTY), DEFAULT_INDEX_FILES_PROPERTY);
        this.enableHtml = OsgiUtil.toBoolean(props.get(HTML_RENDERER_PROPERTY), DEFAULT_RENDERER_PROPERTY);
        this.enableTxt = OsgiUtil.toBoolean(props.get(TXT_RENDERER_PROPERTY), DEFAULT_RENDERER_PROPERTY);
        ...
        this.jsonMaximumResults = OsgiUtil.toInteger(props.get(JSON_RENDERER_MAXIMUM_RESULTS_PROPERTY), DEFAULT_JSON_RENDERER_MAXIMUM_RESULTS);
    }
    ...
    public void init() throws ServletException {
        ...
        setupServlet(rendererMap, StreamRendererServlet.EXT_RES, new StreamRendererServlet(index, indexFiles));
        if (enableHtml) {
            setupServlet(rendererMap, HtmlRendererServlet.EXT_HTML, new HtmlRendererServlet());
        }
        if (enableTxt) {
            setupServlet(rendererMap, PlainTextRendererServlet.EXT_TXT, new PlainTextRendererServlet());
        }
        if (enableJson) {
            setupServlet(rendererMap, JsonRendererServlet.EXT_JSON, new JsonRendererServlet(jsonMaximumResults));
        }
        if (enableXml) {
            setupServlet(rendererMap, XMLRendererServlet.EXT_XML, new XMLRendererServlet());
        }
