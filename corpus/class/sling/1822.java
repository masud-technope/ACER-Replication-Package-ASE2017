...
public abstract class AbstractClusterTest {
    ...
    private void doTestLeader(String slingId1, String slingId2) throws Throwable {
        ...
        instance2.stop();
        instance1.stop();
        instance1 = newBuilder().setDebugName("firstInstance").newRepository("/var/discovery/impl/", true).setConnectorPingTimeout(30).setMinEventDelay(1).setSlingId(slingId1).build();
        ...
        Thread.sleep(200);
        instance2 = newBuilder().setDebugName("secondInstance").useRepositoryOf(instance1).setConnectorPingTimeout(30).setMinEventDelay(1).setSlingId(slingId2).build();
    ...
    public void testDuplicateInstance3726() throws Throwable {
        ...
        for (int i = 0; i < 25; i++) {
            // loop for max 25 times, min 20 times
            runHeartbeatOnceWith(instance2, instance3, /*instance4, */
            instance5);
            final boolean ping1 = pingConnector(instance3, instance2);
            final boolean ping2 = pingConnector(instance5, instance2);
            if (ping1 && ping2) {
                ...
                if (i < 20) {
                    ...
                    // 20x1000ms = 20sec max - (vs 15sec timeout) - should be enough for timing out
                    Thread.sleep(1000);
                    continue;
                }
                break;
            ...
            // 25x1000ms = 25sec max - (vs 15sec timeout)
            Thread.sleep(1000);
        }
        assertTrue(success);
        // one final heartbeat
    ...
    private String prepare4139() throws Throwable, Exception, InterruptedException {
        ...
        tearDown();
        instance1 = newBuilder().setDebugName("firstInstance").newRepository("/var/discovery/impl/", true).setConnectorPingTimeout(Integer.MAX_VALUE).setMinEventDelay(1).build();
        instance2 = newBuilder().setDebugName("secondInstance").useRepositoryOf(instance1).setConnectorPingTimeout(Integer.MAX_VALUE).setMinEventDelay(1).build();
        ...
        // now launch the remote instance
        instance3 = newBuilder().setDebugName("remoteInstance").newRepository("/var/discovery/implremote/", false).setConnectorPingTimeout(Integer.MAX_VALUE).setMinEventDelay(1).build();
        assertSameClusterIds(instance1, instance2);
        try {
        ...
        // and have instance3 no longer pinging instance1
        instance1.stopViewChecker();
        // otherwise it will have itself still registered with the observation manager and fiddle with future events..
        instance1.stop();
        // set to null to early fail if anyone still assumes (original) instance1 is up form now on
        instance1 = null;
        // set instance2's heartbeatTimeout to 1 sec to time out instance1 quickly!
        instance2.getConfig().setViewCheckTimeout(1);
        // set instance3's heartbeatTimeout to 1 sec to time out instance1 quickly!
        instance3.getConfig().setViewCheckTimeout(1);
        Thread.sleep(500);
        ...
        assertTopology(instance3, new SimpleClusterView(instance3));
        // set instance2's heartbeatTimeout back to Integer.MAX_VALUE /* no timeout */
        instance2.getConfig().setViewCheckTimeout(Integer.MAX_VALUE);
        // set instance3's heartbeatTimeout back to Integer.MAX_VALUE /* no timeout */
        instance3.getConfig().setViewCheckTimeout(Integer.MAX_VALUE);
    ...
    private void assertNotSameClusterIds(VirtualInstance... instances) throws UndefinedClusterViewException {
        ...
        final String clusterId1 = instances[0].getClusterViewService().getLocalClusterView().getId();
        for (int i = 1; i < instances.length; i++) {
            final String otherClusterId = instances[i].getClusterViewService().getLocalClusterView().getId();
    ...
    private void registerReplyAnnouncement(VirtualInstance from, Announcement inheritedAnnouncement) {
        ...
        if (inheritedAnnouncement.isLoop()) {
            ...
            if (announcementRegistry.registerAnnouncement(inheritedAnnouncement) == -1) {
                logger.info("ping: connector response is from an instance which I already see in my topology" + inheritedAnnouncement);
                return;
            }
        }
    ...
    public void testStableClusterId() throws Throwable {
        ...
        // would actually not be necessary as it was never started.. this test only runs heartbeats manually
        instance2.stopViewChecker();
        instance1.heartbeatsAndCheckView();
        Thread.sleep(500);
        instance1.heartbeatsAndCheckView();
    ...
    private InstanceDescription getInstance(VirtualInstance instance, String slingId) throws UndefinedClusterViewException {
        Iterator<InstanceDescription> it = instance.getClusterViewService().getLocalClusterView().getInstances().iterator();
        while (it.hasNext()) {
