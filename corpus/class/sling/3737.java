...
public class ValidationModelRetrieverImpl implements ValidationModelRetriever, EventHandler {
    ...
    public ValidationModel getModel(@Nonnull String resourceType, String resourcePath, boolean considerResourceSuperTypeModels) {
        try {
            ...
            throw new IllegalStateException("Could not retrieve models, because the administrative resource resolver could not be acquired", e);
        }
    }

    @CheckForNull
    ValidationModel getModel(@Nonnull String resourceType, String resourcePath, boolean considerResourceSuperTypeModels, @Nonnull ResourceResolver resourceResolver) {
    ...
    @CheckForNull
    private ValidationModel getModel(@Nonnull String resourceType, String resourcePath, @Nonnull ResourceResolver resourceResolver) {
        ...
        if (model == null && !modelsForResourceType.isEmpty()) {
            LOG.warn("Although model for resource type {} is available, it is not allowed for path {}", resourceType, resourcePath);
        }
        return model;
    }

    @Nonnull
    private synchronized Trie<ValidationModel> fillTrieForResourceType(@Nonnull String resourceType, @Nonnull ResourceResolver resourceResolver) {
        Trie<ValidationModel> modelsForResourceType = validationModelsCache.get(resourceType);
    ...
    protected void bindValidator(Validator<?> validator) {
        if (validators.put(validator.getClass().getName(), validator) != null) {
            LOG.debug("Validator with the name '{}' has been registered in the system already and was now overwritten", validator.getClass().getName());
        }
    }
