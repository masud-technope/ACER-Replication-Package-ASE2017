...
public class ServletFilterManager extends ServiceTracker {
    ...
    public static enum FilterChainType implements  {
    ...
    public  ServletFilterManager(final BundleContext context, final SlingServletContext servletContext, final boolean compatMode) {
    ...
    private boolean excludeFilter(final ServiceReference reference) {
        ...
        if (reference.getProperty(EngineConstants.SLING_FILTER_SCOPE) != null || reference.getProperty(EngineConstants.FILTER_SCOPE) != null) {
            return false;
        }
    ...
    private void initFilter(final ServiceReference reference, final Filter filter) {
                ...
                if (scopes != null && scopes.length > 0) {
                    for (String scope : scopes) {
                        ...
                        try {
                            ...
                            getFilterChain(type).addFilter(filter, pattern, serviceId, order, orderSource, mbean);
                            if (type == FilterChainType.COMPONENT) {
                                getFilterChain(FilterChainType.INCLUDE).addFilter(filter, pattern, serviceId, order, orderSource, mbean);
                                getFilterChain(FilterChainType.FORWARD).addFilter(filter, pattern, serviceId, order, orderSource, mbean);
                            }
                        } catch (IllegalArgumentException iae) {
                        }
                    }
                } else {
                    log.warn(String.format("A Filter (Service ID %s) has been registered without a filter.scope property.", reference.getProperty(Constants.SERVICE_ID)));
                    getFilterChain(FilterChainType.REQUEST).addFilter(filter, pattern, serviceId, order, orderSource, mbean);
                }
    ...
    private void destroyFilter(final ServiceReference reference, final Filter filter) {
        // service id
