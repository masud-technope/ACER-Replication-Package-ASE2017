...
public class WebconsoleClient {
    ...
    public void startBundle(String symbolicName) throws Exception {
        ...
        executor.execute(builder.buildPostRequest(path).withCredentials(username, password).withEntity(entity)).assertStatus(200);
    }

    private String getBundlePath(String symbolicName, String extension) {
        return CONSOLE_BUNDLES_PATH + "/" + symbolicName + (extension == null ? "" : extension);
    }
    ...
    public void refreshPackages() throws Exception {
        ...
        executor.execute(builder.buildPostRequest(CONSOLE_BUNDLES_PATH).withCredentials(username, password).withEntity(entity)).assertStatus(200);
    }
}
