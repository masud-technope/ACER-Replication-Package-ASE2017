...
public class SlingPostServlet extends SlingAllMethodsServlet {
    ...
    @Property({ "title", "jcr:title", "name", "description", "jcr:description", "abstract", "text", "jcr:text" })
     ...
     * @throws IOException
     */
    boolean redirectIfNeeded(final SlingHttpServletRequest request, final PostResponse htmlResponse, final SlingHttpServletResponse response) throws IOException {
    ...
    PostResponse createPostResponse(final SlingHttpServletRequest req) {
        ...
        @SuppressWarnings({ "MismatchedQueryAndUpdateOfCollection" }) final MediaRangeList mediaRangeList = new MediaRangeList(req);
    ...
    private PostOperation getSlingPostOperation(final SlingHttpServletRequest request) {
    ...
    protected String getRedirectUrl(final SlingHttpServletRequest request, final PostResponse ctx) {
        ...
        if (result != null) {
            ...
            if (star >= 0 && ctx.getPath() != null) {
                ...
                if (requestPath.endsWith(SlingPostConstants.DEFAULT_CREATE_SUFFIX) && buf.charAt(0) != '/' && !REDIRECT_WITH_SCHEME_PATTERN.matcher(buf).matches()) {
    ...
    protected boolean isSetStatus(final SlingHttpServletRequest request) {
        ...
        if (statusParam == null) {
            log.debug("getStatusMode: Parameter {} not set, assuming standard status code", SlingPostConstants.RP_STATUS);
            return true;
        }
        ...
        log.debug("getStatusMode: Parameter {} set to unknown value {}, assuming standard status code", SlingPostConstants.RP_STATUS);
        return true;
    }
    ...
    protected void activate(final ComponentContext context, final Map<String, Object> configuration) {
        ...
        providedServices.add(registerOperation(bundleContext, SlingPostConstants.OPERATION_DELETE, new DeleteOperation()));
        providedServices.add(registerOperation(bundleContext, SlingPostConstants.OPERATION_NOP, new NopOperation()));
        providedServices.add(registerOperation(bundleContext, SlingPostConstants.OPERATION_CHECKIN, new CheckinOperation()));
        providedServices.add(registerOperation(bundleContext, SlingPostConstants.OPERATION_CHECKOUT, new CheckoutOperation()));
        providedServices.add(registerOperation(bundleContext, SlingPostConstants.OPERATION_RESTORE, new RestoreOperation()));
        providedServices.add(registerOperation(bundleContext, SlingPostConstants.OPERATION_IMPORT, importOperation));
        internalOperations = providedServices.toArray(new ServiceRegistration[providedServices.size()]);
    }

    private ServiceRegistration registerOperation(final BundleContext context, final String opCode, final PostOperation operation) {
        ...
        properties.put(Constants.SERVICE_VENDOR, context.getBundle().getHeaders().get(Constants.BUNDLE_VENDOR));
        return context.registerService(PostOperation.SERVICE_NAME, operation, properties);
    }
    ...
    private void configure(final Map<String, Object> configuration) {
        ...
        final int nameMax = (int) OsgiUtil.toLong(configuration.get(PROP_NODE_NAME_MAX_LENGTH), -1);
        final NodeNameGenerator nodeNameGenerator = new DefaultNodeNameGenerator(nameHints, nameMax);
        final String paramMatch = OsgiUtil.toString(configuration.get(PROP_IGNORED_PARAMETER_NAME_PATTERN), DEFAULT_IGNORED_PARAMETER_NAME_PATTERN);
    ...
    protected void bindNodeNameGenerator(final NodeNameGenerator generator, final Map<String, Object> properties) {
        ...
        synchronized (this.nodeNameGenerators) {
            int index = 0;
            while (index < this.nodeNameGenerators.size() && nngh.ranking < this.nodeNameGenerators.get(index).ranking) {
                index++;
            }
    ...
    protected void bindPostResponseCreator(final PostResponseCreator creator, final Map<String, Object> properties) {
        ...
        synchronized (this.postResponseCreators) {
            int index = 0;
            while (index < this.postResponseCreators.size() && nngh.ranking < this.postResponseCreators.get(index).ranking) {
                index++;
            }
    ...
    private VersioningConfiguration createBaseVersioningConfiguration(Map<?, ?> props) {
        ...
        cfg.setCheckinOnNewVersionableNode(OsgiUtil.toBoolean(props.get(PROP_CHECKIN_ON_CREATE), DEFAULT_CHECKIN_ON_CREATE));
        cfg.setAutoCheckout(OsgiUtil.toBoolean(props.get(PROP_AUTO_CHECKOUT), DEFAULT_AUTO_CHECKOUT));
        cfg.setAutoCheckin(OsgiUtil.toBoolean(props.get(PROP_AUTO_CHECKIN), DEFAULT_AUTO_CHECKIN));
        return cfg;
    }
