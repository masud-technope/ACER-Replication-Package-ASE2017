...
public class SessionProxyHandler {
    ...
    public Session createProxy(final Session session) {
        ...
        return (Session) Proxy.newProxyInstance(sessionClass.getClassLoader(), interfaces, new SessionProxyInvocationHandler(session, this.namespaceSupport, interfaces));
    }
    ...
    public static final class SessionProxyInvocationHandler implements InvocationHandler {
        ...
        public  SessionProxyInvocationHandler(final Session delegatee, final NamespaceMappingSupport namespaceSupport, final Class<?>[] interfaces) {
        ...
        public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
            if (method.getName().equals("impersonate") && args != null && args.length == 1) {
                ...
                final Class<?> sessionClass = session.getClass();
                return Proxy.newProxyInstance(sessionClass.getClassLoader(), interfaces, new SessionProxyInvocationHandler(session, this.namespaceSupport, interfaces));
            }
            try {
     ...
     */
    private void guessWorkInterfaces(final Class<?> clazz, final Set<Class<?>> workInterfaces) {
        if (null != clazz) {
     ...
     * @param workInterfaces  the set of current work interfaces
     */
    private void addInterfaces(final Class<?>[] classInterfaces, final Set<Class<?>> workInterfaces) {
