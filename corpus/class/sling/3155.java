...
public interface Job {
     ...
     * @since 1.3
     */
    enum JobState implements  {

        // waiting in queue after adding or for restart after failing
        QUEUED() {
        }
        , // job is currently in processing
        ACTIVE() {
        }
        , // processing finished successfully
        SUCCEEDED() {
        }
        , // processing was stopped by a user
        STOPPED() {
        }
        , // number of retries reached
        GIVEN_UP() {
        }
        , // processing signaled CANCELLED or throw an exception
        ERROR() {
        }
        , // dropped jobs
        DROPPED() {
