...
public class SlingUserManager implements UserManager {
    ...
    public User authenticate(Authentication auth) throws AuthenticationFailedException {
        ...
        try {
            if (auth instanceof AnonymousAuthentication) {
                ...
                creds.put(ResourceResolverFactory.PASSWORD, ((UsernamePasswordAuthentication) auth).getPassword().toCharArray());
                resolver = this.rrFactory.getResourceResolver(creds);
            } else {
                resolver = null;
            }
            if (resolver != null) {
                return createUser(resolver.getUserID(), resolver.adaptTo(org.apache.jackrabbit.api.security.user.User.class), resolver);
            }
    ...
    public String[] getAllUserNames() throws FtpException {
        ...
        try {
            ...
            if (um != null) {
                try {
                    Iterator<Authorizable> ai = um.findAuthorizables(null, null, org.apache.jackrabbit.api.security.user.UserManager.SEARCH_TYPE_USER);
    ...
    private User createUser(final String name, final org.apache.jackrabbit.api.security.user.User repoUser, final ResourceResolver resolver) {
        ...
        getProperty(repoUser, FTP_MAX_CONCURRENT, config.getMaxConcurrent()), getProperty(repoUser, FTP_MAX_CONCURRENT_PER_IP, config.getMaxConcurrentPerIp())));
        list.add(new //
        TransferRatePermission(//
        getProperty(repoUser, FTP_MAX_DOWNLOAD_RATE, config.getMaxDownloadRate()), getProperty(repoUser, FTP_MAX_UPLOAD_RATE, config.getMaxUploadRate())));
