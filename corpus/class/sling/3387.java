...
public class CassandraResource extends AbstractResource {
    ...
    public Resource getParent() {
        return new CassandraResource(this.resourceProvider, this.resourceResolver, CassandraResourceProviderUtil.getParentPath(resourcePath), valueMap);
    }
    ...
    public Iterator<Resource> listChildren() {
        ...
        try {
            QueryResult<CqlRows<String, String, String>> result = CassandraResourceProviderUtil.getAllNodes(resourceProvider.getKeyspace(), CassandraResourceProviderUtil.getColumnFamilySector(resourcePath));
            for (Row<String, String, String> row : result.get().getList()) {
                for (HColumn column : row.getColumnSlice().getColumns()) {
    ...
    public Resource getChild(String s) {
        return resourceProvider.getResource(resourceResolver, new StringBuilder(resourcePath.endsWith("/") ? resourcePath.substring(0, resourcePath.length() - 1) : resourcePath).append("/").append(s).toString());
    ...
    public String toString() {
        return new StringBuilder("{\nresourcePath=").append(resourcePath).append("\n    remainingPath=").append(remainingPath).append("\n    columnFamilySector=").append(columnFamilySector).append("\n    resourceSuperType=").append(resourceSuperType).append("\n    resourceType=").append(resourceType).append("\n    metaData=").append(metadata).append("\n}").toString();
    }
}
