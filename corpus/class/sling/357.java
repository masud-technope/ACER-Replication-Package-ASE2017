...
public class JsCodeGeneratorTest extends TestCase {
    ...
    protected boolean runTest(int index) throws Exception {
        ...
        final String expectedCode = StringUtil.readClassResource(getClass(), TEST_RESOURCES_PATH + "/jscode" + index + ".expected.js", ENCODING);
        final StringWriter result = new StringWriter();
        generator.generateCode(r, new PrintWriter(result));
        assertEquals("Generated code must match expected code for " + input, TestStringUtil.flatten(expectedCode.toString().trim()), TestStringUtil.flatten(result.toString().trim()));
        return true;
    }
