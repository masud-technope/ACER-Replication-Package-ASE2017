...
@Component
@Service
@Properties({ @Property(name = "service.description", value = "Sightly Resource Block Plugin"), @Property(name = Plugin.SCR_PROP_NAME_BLOCK_NAME, value = "attribute"), @Property(name = Plugin.SCR_PROP_NAME_PRIORITY, intValue = 150) })
public class AttributePlugin extends PluginComponent {
    ...
    private final class SingleAttributeInvoke extends DefaultPluginInvoke {
        ...
        private void emitStart(PushStream stream) {
            ...
            stream.emit(new VariableBinding.Start(shouldDisplayAttribute, new BinaryOperation(BinaryOperator.OR, new Identifier(escapedAttrValue), new BinaryOperation(BinaryOperator.EQ, new StringConstant("false"), new Identifier(attrValue)))));
            stream.emit(new Conditional.Start(shouldDisplayAttribute, true));
        }

        private void emitWrite(PushStream stream) {
            stream.emit(new VariableBinding.Start(isTrueValue, new BinaryOperation(BinaryOperator.EQ, new Identifier(attrValue), BooleanConstant.TRUE)));
    ...
    private final class MultiAttributeInvoke extends DefaultPluginInvoke {
        ...
        private void writeAttribute(PushStream stream, String attrNameVar, String attrContentVar) {
            ...
            stream.emit(new VariableBinding.Start(shouldDisplayAttribute, new BinaryOperation(BinaryOperator.OR, new Identifier(escapedContent), new BinaryOperation(BinaryOperator.EQ, new StringConstant("false"), new Identifier(attrContentVar)))));
            stream.emit(new Conditional.Start(shouldDisplayAttribute, true));
            //write("attrName");
            stream.emit(new OutText(" "));
        ...
        private void writeAttributeValue(PushStream stream, String escapedContent, String attrContentVar) {
            ...
            String isTrueVar = compilerContext.generateVariable("isTrueAttr");
            stream.emit(new //isTrueAttr = (attrContent == true)
            VariableBinding.Start(//isTrueAttr = (attrContent == true)
            isTrueVar, new BinaryOperation(BinaryOperator.EQ, new Identifier(attrContentVar), BooleanConstant.TRUE)));
            //if (!isTrueAttr)
            stream.emit(new Conditional.Start(isTrueVar, false));
            stream.emit(new OutText("=\""));
            //write(escapedContent)
            stream.emit(new OutVariable(escapedContent));
            stream.emit(new OutText("\""));
            //end if isTrueAttr
            stream.emit(Conditional.END);
