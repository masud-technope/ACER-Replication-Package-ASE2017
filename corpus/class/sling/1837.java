...
abstract class AbstractItemDescriptor extends AbstractDescriptorElement {
    ...
    public  AbstractItemDescriptor(Log log, DocletTag tag, String fieldName) {
        ...
        isJcrAutoCreated = Boolean.valueOf(tag.getNamedParameter(JCR_AUTO_CREATED)).booleanValue();
    ...
    boolean validate() {
        ...
        if (jcrOnParentVersion != null) {
            if (!jcrOnParentVersion.equals("COPY") && !jcrOnParentVersion.equals("VERSION") && !jcrOnParentVersion.equals("INITIALIZE") && !jcrOnParentVersion.equals("COMPUTE") && !jcrOnParentVersion.equals("IGNORE") && !jcrOnParentVersion.equals("ABORT")) {
    ...
    static String getFieldFromMethod(JavaMethod method) {
        ...
        if ((fieldName.startsWith("get") || fieldName.startsWith("set")) && fieldName.length() >= 4) {
