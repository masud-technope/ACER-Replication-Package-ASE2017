...
public class OakResourceListener extends NodeObserver implements Closeable {
    ...
    public  OakResourceListener(final String mountPrefix, final ObservationListenerSupport support, final BundleContext bundleContext, final Executor executor, final PathMapper pathMapper, final int observationQueueLength) throws RepositoryException {
        ...
        final Observer observer = new BackgroundObserver(this, executor, observationQueueLength) {
            ...
            protected void added(int queueSize) {
                if (queueSize == observationQueueLength) {
                    logger.warn("Revision queue for observer {} is full (max = {}). Further revisions will be compacted.", getClass().getName(), observationQueueLength);
                }
            }
        };
    ...
    @Override
    protected void added(final String path, final Set<String> added, final Set<String> deleted, final Set<String> changed, final Map<String, String> properties, final CommitInfo commitInfo) {
    ...
    @Override
    protected void deleted(final String path, final Set<String> added, final Set<String> deleted, final Set<String> changed, final Map<String, String> properties, final CommitInfo commitInfo) {
        final Map<String, Object> changes = toEventProperties(added, deleted, changed);
        addCommitInfo(changes, commitInfo);
        if (logger.isDebugEnabled()) {
    ...
    @Override
    protected void changed(final String path, final Set<String> added, final Set<String> deleted, final Set<String> changed, final Map<String, String> properties, final CommitInfo commitInfo) {
        final Map<String, Object> changes = toEventProperties(added, deleted, changed);
        addCommitInfo(changes, commitInfo);
        if (logger.isDebugEnabled()) {
    ...
    private void sendOsgiEvent(final String path, final String topic, final Map<String, Object> changes, final Map<String, String> properties) {
                ...
                if (!TOPIC_RESOURCE_REMOVED.equals(topic)) {
                    ...
                    if (path.endsWith("/jcr:content")) {
                        ...
                        if (!sendEvent) {
                            ...
                            logger.debug("processOsgiEventQueue: Resource at {} not found, which is not expected for an added or modified node", changePath);
                        }
                    }
