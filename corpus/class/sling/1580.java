...
final class OsgiServiceUtil {
    ...
    public static boolean activateDeactivate(Object target, ComponentContext componentContext, boolean activate) {
        ...
        if (invokeLifecycleMethod(target, targetClass, methodName, !activate, componentContext, MapUtil.toMap(componentContext.getProperties()))) {
            return true;
        }
    ...
    private static boolean invokeLifecycleMethod(Object target, Class<?> targetClass, String methodName, boolean allowIntegerArgument, ComponentContext componentContext, Map<String, Object> properties) {
        ...
        if (method != null) {
            invokeMethod(target, method, new Object[] { Annotations.toObject(method.getParameterTypes()[0], MapUtil.toMap(componentContext.getProperties()), componentContext.getBundleContext().getBundle(), false) });
            return true;
        }
        ...
        // 7. mixed arguments
        Class<?>[] mixedArgsAllowed = allowIntegerArgument ? new Class<?>[] { ComponentContext.class, BundleContext.class, Map.class, Annotation.class, int.class, Integer.class } : new Class<?>[] { ComponentContext.class, BundleContext.class, Map.class, Annotation.class };
        ...
        for (Method method : methods) {
            if (StringUtils.equals(method.getName(), methodName) && method.getParameterTypes().length == types.length) {
                ...
                for (int i = 0; i < types.length; i++) {
                    if (!((method.getParameterTypes()[i] == types[i]) || (types[i] == Annotation.class && method.getParameterTypes()[i].isAnnotation()))) {
                        foundMismatch = true;
                        break;
                    }
                }
        ...
        for (Method method : methods) {
            if (StringUtils.equals(method.getName(), methodName) && method.getParameterTypes().length > 1) {
                ...
                for (Class<?> parameterType : method.getParameterTypes()) {
                    ...
                    for (int i = 0; i < types.length; i++) {
                        if ((parameterType == types[i]) || (types[i] == Annotation.class && parameterType.isAnnotation())) {
                            foundAnyMatch = true;
                            break;
                        }
                    }
    ...
    private static void invokeBindUnbindMethod(Reference reference, Object target, ServiceInfo serviceInfo, boolean bind) {
        ...
        } else // in OSGi declarative services 1.3 there are no bind/unbind methods - modify the field directly
            ...
            if (reference.isCardinalityMultiple()) {
                switch(reference.getFieldCollectionType()) {
                    ...
                    case REFERENCE:
                        ...
                        break;
                    default:
                        throw new RuntimeException("Field collection type '" + reference.getFieldCollectionType() + "' not supported " + "for reference '" + reference.getName() + "' for class " + targetClass.getName());
                }
            } else // check for single field reference
            {
