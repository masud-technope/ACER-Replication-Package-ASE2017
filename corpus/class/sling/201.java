...
abstract class JcrInstallTestBase extends RepositoryTestBase {
    ...
    protected void assertRegistered(String info, String path, boolean registered) {
        ...
        if (registered) {
            assertTrue(info + "Expected " + path + " to be registered", osgiInstaller.isRegistered(JcrInstaller.URL_SCHEME, path));
        } else {
            assertFalse(info + "Expected " + path + " to be unregistered", osgiInstaller.isRegistered(JcrInstaller.URL_SCHEME, path));
        }
    }
