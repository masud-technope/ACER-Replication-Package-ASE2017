...
public class ValidationServiceImpl implements ValidationService {
    ...
    protected String getRelativeResourceType(@Nonnull String resourceType) throws IllegalArgumentException {
        if (resourceType.startsWith("/")) {
            ...
            throw new IllegalArgumentException("Can only deal with resource types inside the resource resolver's search path (" + StringUtils.join(searchPaths.toArray()) + ") but given resource type " + resourceType + " is outside!");
        }
        return resourceType;
    }
    ...
    @Override
    @Nonnull
    public ValidationResult validateResourceRecursively(@Nonnull Resource resource, boolean enforceValidation, Predicate filter, boolean considerResourceSuperTypeModels) throws IllegalStateException, IllegalArgumentException, SlingValidationException {
    ...
    private void validateValueMap(ValueMap valueMap, Resource resource, String relativePath, Collection<ResourceProperty> resourceProperties, ValidationResultImpl result) {
        if (valueMap == null) {
    ...
    private void validateValue(ValidationResultImpl result, @Nonnull Object value, String property, String relativePath, @Nonnull ValueMap valueMap, Resource resource, ParameterizedValidator validator) {
        try {
            @SuppressWarnings("unchecked") String validatorMessage = ((Validator) validator.getValidator()).validate(value, valueMap, resource, validator.getParameters());
            if (validatorMessage != null) {
                if (validatorMessage.isEmpty()) {
                    validatorMessage = "Property does not contain a valid value for the " + validator.getClass().getName() + " validator";
                }
