...
public class BasicObservationReporter implements ObservationReporter {
     ...
     */
    public  BasicObservationReporter(final Collection<ResourceChangeListenerInfo> infos, final Path providerPath, final PathSet excludePaths) {
        ...
        for (final Map.Entry<String, ObserverConfig> entry : configMap.entrySet()) {
            final ObserverConfiguration cfg = new BasicObserverConfiguration(entry.getKey(), entry.getValue().types, entry.getValue().isExternal, excludePaths);
            result.add(cfg);
        }
