...
public class RepositoryTestHelper {
    ...
    public static void dumpRepo(ResourceResolverFactory resourceResolverFactory) throws Exception {
        Session session = resourceResolverFactory.getAdministrativeResourceResolver(null).adaptTo(Session.class);
    ...
    public static Repository createOakRepository(NodeStore nodeStore) {
        ...
        final Oak oak = new Oak(nodeStore).with(new InitialContent()).with(JcrConflictHandler.createJcrConflictHandler()).with(new EditorHook(new VersionEditorProvider())).with(new OpenSecurityProvider()).with(//        })
        new NamespaceEditorProvider()).with(new TypeEditorProvider()).with(//        .with(new RegistrationEditorProvider())
        new ConflictValidatorProvider()).with(//        .with(indexEditorProvider)
        "default").with(//        .withAsyncIndexing()
        whiteboard);
    ...
    public static ResourceResolverFactory mockResourceResolverFactory(final SlingRepository repositoryOrNull) throws Exception {
        ...
        context.checking(new Expectations() {
            ...
            {
                allowing(resourceResolverFactory).getAdministrativeResourceResolver(null);
                will(new Action() {
