...
public class AdvancedRemoteDistributionPackageImporter implements DistributionPackageImporter {
    ...
    protected void activate(Map<String, ?> config) throws Exception {
        ...
        String endpointStrategyName = PropertiesUtil.toString(config.get(ENDPOINT_STRATEGY), TransportEndpointStrategyType.One.name());
        ...
        for (String endpoint : endpoints) {
            if (endpoint != null && endpoint.length() > 0) {
                transportHandlers.add(new AdvancedHttpDistributionTransport(distributionLog, useCustomHeaders, customHeaders, useCustomBody, customBody, new DistributionEndpoint(endpoint), null, distributionTransportSecretProvider, -1));
            }
        }
        transportHandler = new MultipleEndpointDistributionTransport(transportHandlers, transportEndpointStrategyType);
    }
