...
public class XSSFilterImpl implements XSSFilter, EventHandler {
    ...
    static final Attribute DEFAULT_HREF_ATTRIBUTE = new Attribute("href", Arrays.asList(Pattern.compile("([\\p{L}\\p{N}\\\\\\.\\#@\\$%\\+&;\\-_~,\\?=/!\\*\\(\\)]*|\\#(\\w)+)"), Pattern.compile("(\\s)*((ht|f)tp(s?)://|mailto:)[\\p{L}\\p{N}]+[\\p{L}\\p{N}\\p{Zs}\\.\\#@\\$%\\+&;:\\-_~,\\?=/!\\*\\(\\)]*(\\s)*")), Collections.<String>emptyList(), "removeAttribute", "");
    ...
    private void updateDefaultHandler() {
        ...
        try {
            ...
            if (policyResource != null) {
                ...
                LOGGER.debug("Could not find a policy file at the default location {}. Attempting to use the default resource embedded in" + " the bundle.", DEFAULT_POLICY_PATH);
