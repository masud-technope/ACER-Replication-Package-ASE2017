...
public class ForwardDistributionAgentFactory extends AbstractDistributionAgentFactory {
    ...
    @Property(boolValue = false, label = "Use multiple queues", description = "Whether or not to use an individual queue for each importer endpoint. " + "If the queue names are not specified by importer endpoints definition then they are autogenerated.")
    ...
    @Property(cardinality = 100, label = "Selective queues", description = "List of selective queues that should used for specific paths." + "The selector format is  {queuePrefix}|{mainQueueMatcher}={pathMatcher}, e.g. french|publish.*=/content/fr.*")
