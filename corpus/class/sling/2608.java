...
public class RequestParameterSupportConfigurer {
    ...
    private void configure(ComponentContext context) {
        ...
        final String fileLocation = getFileLocation(context, PropertiesUtil.toString(props.get(PROP_FILE_LOCATION), null));
    ...
    private String getFileLocation(final ComponentContext context, String fileLocation) {
        if (fileLocation != null) {
            ...
            if (file.exists()) {
                if (!file.isDirectory()) {
                    log.error("Configured temporary file location {} exists but is not a directory; using java.io.tmpdir instead", fileLocation);
                    fileLocation = null;
                }
            } else {
                if (!file.mkdirs()) {
