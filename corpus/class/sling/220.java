...
public class ArtifactGroup extends ItemList<Artifact> implements Comparable<ArtifactGroup> {
    ...
    public Artifact search(final Artifact template) {
        ...
        for (final Artifact current : this) {
            if (current.getGroupId().equals(template.getGroupId()) && current.getArtifactId().equals(template.getArtifactId()) && ((current.getClassifier() == null && template.getClassifier() == null) || (current.getClassifier().equals(template.getClassifier()))) && current.getType().equals(template.getType())) {
                found = current;
                break;
            }
        }
    ...
    public String toString() {
        return "ArtifactGroup [level=" + level + ", artifacts=" + this.items + (this.getLocation() != null ? ", location=" + this.getLocation() : "") + "]";
    }
}
