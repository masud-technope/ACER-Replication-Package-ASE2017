    ...
    private void attachFilter(FilterInfo fi, Map<String, Appender<ILoggingEvent>> appenderMap) {
        ...
        for (String appenderName : fi.appenderNames) {
            ...
            if (appender != null) {
                ...
                contextUtil.addWarn("No appender with name [" + appenderName + "] found " + "to which " + fi.filter + " can be attached");
            }
        }
    }
    ...
    public static class FilterInfo {
        ...
        final boolean registerAgainstAllAppenders;
         ...
         FilterInfo(ServiceReference reference, Filter<ILoggingEvent> filter) {
            ...
            this.filter = filter;
            this.appenderNames = Collections.unmodifiableSet(new HashSet<String>(Util.toList(reference.getProperty(PROP_APPENDER))));
