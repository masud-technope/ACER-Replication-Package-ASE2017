...
public class SlingTestBase implements SlingInstance {
    ...
    public  SlingTestBase(SlingInstanceState slingTestState, Properties systemProperties) {
        ...
        if (configuredUrl != null && configuredUrl.trim().length() > 0) {
            ...
            // never undeploy additional bundles in case the server is provisioned here!
            uninstallAdditionalBundles = false;
        }
        ...
        if (slingTestState.isInstallBundlesFailed()) {
            ...
            if (!toInstall.isEmpty()) {
                try {
                    ...
                    bundlesInstaller.waitForBundlesInstalled(symbolicNames, TimeoutsProvider.getInstance().getTimeout(BUNDLE_INSTALL_TIMEOUT_SECONDS, 10));
                    bundlesInstaller.startAllBundles(symbolicNames, TimeoutsProvider.getInstance().getTimeout(START_BUNDLES_TIMEOUT_SECONDS, 30));
                ...
                log.info("Not installing additional bundles, probably System property {} not set", ADDITONAL_BUNDLES_PATH);
            }
        }
    ...
    protected void waitForServerReady() throws Exception {
        ...
        while (System.currentTimeMillis() < endTime) {
            ...
            for (String p : testPaths) {
                ...
                try {
                    executor.execute(builder.buildGetRequest(path).withCredentials(serverUsername, serverPassword)).assertStatus(200).assertContentContains(pattern);
                } catch (AssertionError ae) {
                    errors = true;
                    log.debug("Request to {}@{}{} failed, will retry ({})", new Object[] { serverUsername, slingTestState.getServerBaseUrl(), path, ae });
                } catch (Exception e) {
                    errors = true;
                    log.debug("Request to {}@{}{} failed, will retry ({})", new Object[] { serverUsername, slingTestState.getServerBaseUrl(), path, pattern, e });
                }
            }
            if (!errors) {
