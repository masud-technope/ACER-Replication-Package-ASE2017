...
public class MarkupHandler {
    ...
    private void emitMultipleFragment(String name, Interpolation interpolation, PluginInvoke invoke) {
        ...
        stream.emit(new VariableBinding.Start(shouldDisplayAttr, new BinaryOperation(BinaryOperator.OR, new Identifier(attrContent), new BinaryOperation(BinaryOperator.EQ, new StringConstant("false"), new Identifier(attrContent)))));
    ...
    private void emitSingleFragment(String name, Interpolation interpolation, PluginInvoke invoke) {
        ...
        stream.emit(new VariableBinding.Start(attrContent, expressionWrapper.adjustToContext(contentExpression, markupContext, ExpressionContext.ATTRIBUTE).getRoot()));
        stream.emit(new VariableBinding.Start(shouldDisplayAttr, new BinaryOperation(BinaryOperator.OR, new Identifier(attrContent), new BinaryOperation(BinaryOperator.EQ, new StringConstant("false"), new Identifier(attrValue)))));
        // if (attrContent)
        stream.emit(new Conditional.Start(shouldDisplayAttr, true));
        //write("attrName");
        emitAttributeStart(name);
        invoke.beforeAttributeValue(stream, name, node);
        stream.emit(new //isTrueAttr = (attrValue == true)
        ...
        for (Fragment fragment : interpolation.getFragments()) {
            ...
            if (fragment.isString()) {
                ...
                if (fragment.getExpression().containsOption(Syntax.CONTEXT_OPTION)) {
                    ...
                    log.warn("Element {} requires that all expressions have an explicit context specified. Expression will be " + "replaced by the empty string", currentTag);
        ...
        for (Fragment fragment : interpolation.getFragments()) {
            ...
            if (fragment.isExpression()) {
                ...
                if (!expression.containsOption(Syntax.CONTEXT_OPTION)) {
                    log.warn("All expressions within the value of attribute {} need to have an explicit context option. The expression will be erased.", attributeName);
