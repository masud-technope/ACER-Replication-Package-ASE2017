...
public class ServerSideTestClient extends SlingClient {
    ...
    private static final String configuredUrl = System.getProperty(SlingTestBase.TEST_SERVER_URL_PROP, System.getProperty("launchpad.http.server.url"));
    ...
    public void assertTestsPass(String testPackageOrClassName, int expectedTestsCount) throws Exception {
        ...
        if (expectedTestsCount < 0) {
            assertTrue("Expecting at least " + -expectedTestsCount + " test(s) for " + testPackageOrClassName, results.getTestCount() >= -expectedTestsCount);
        } else {
