 ...
 */
public class LogSupport implements SynchronousBundleListener, ServiceListener, FrameworkListener {
     ...
     */
    // "component.name";
    private static final String COMPONENT_NAME = ComponentConstants.COMPONENT_NAME;
    ...
    private final Map<Long, Logger> loggers = new LinkedHashMap<Long, Logger>(16, 0.75f, true) {
    ...
    /* package */
     LogSupport(final StartLevel startLevelService) {
    ...
    void removeLogListener(LogListener listener) {
        synchronized (listenersLock) {
            ...
            if (idx < listeners.length) {
                System.arraycopy(listeners, idx + 1, newListeners, 0, newListeners.length - idx);
            }
            listeners = newListeners;
        }
    }
    ...
    static int getLevel(Logger log) {
        if (log.isTraceEnabled())
            // No constant for trace in LogService
            return LogService.LOG_DEBUG + 1;
    ...
    private static class LogListenerProxy implements LogListener {
        ...
        private final int runningBundle = Bundle.STARTING | Bundle.ACTIVE | Bundle.STOPPING;
