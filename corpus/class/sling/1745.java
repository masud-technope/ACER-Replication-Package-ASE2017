...
public class ChaosTest extends AbstractJobHandlingTest {
    ...
    private void setupJobConsumers(final List<ServiceRegistration> registrations) {
        for (int i = 0; i < NUM_ORDERED_TOPICS; i++) {
            registrations.add(this.registerJobConsumer(ORDERED_TOPICS[i], new JobConsumer() {
        ...
        for (int i = 0; i < NUM_PARALLEL_TOPICS; i++) {
            registrations.add(this.registerJobConsumer(PARALLEL_TOPICS[i], new JobConsumer() {
        ...
        for (int i = 0; i < NUM_ROUND_TOPICS; i++) {
            registrations.add(this.registerJobConsumer(ROUND_TOPICS[i], new JobConsumer() {
    ...
    private static final class CreateJobThread extends Thread {
        ...
        public  CreateJobThread(final JobManager jobManager, final String[] topics, final Map<String, AtomicLong> created, final AtomicLong finishedThreads) {
            this.topics = topics;
     ...
     */
    private void setupJobCreationThreads(final List<Thread> threads, final JobManager jobManager, final Map<String, AtomicLong> created, final AtomicLong finishedThreads) {
    ...
    public void testDoChaos() throws Exception {
        ...
        final ServiceRegistration eventHandler = this.registerEventHandler("org/apache/sling/event/notification/job/*", new EventHandler() {
