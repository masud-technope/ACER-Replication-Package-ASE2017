    ...
    public void flush() throws IOException {
        ...
        try {
            if (!node.getSession().isLive()) {
                ...
                final Node streamNode = new DeepNodeCreator().deepCreateNode(streamNodePath, node.getSession(), STREAM_NODE_TYPE);
                streamNode.setProperty(NodeStreamPath.PROPERTY_NAME, new ByteArrayInputStream(buffer.toByteArray()));
                log.debug("Saved {} bytes to Property {}", buffer.size(), streamNode.getProperty(NodeStreamPath.PROPERTY_NAME).getPath());
                node.save();
                buffer.reset();
            }
