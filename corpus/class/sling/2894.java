...
public class JcrProviderStateFactory {
            ...
            if (Boolean.TRUE.equals(authenticationInfo.get(ResourceProvider.AUTH_ADMIN))) {
                ...
                if (session == null) {
                    ...
                    if (serviceBundleObject instanceof Bundle) {
                        final String subServiceName = (authenticationInfo.get(ResourceResolverFactory.SUBSERVICE) instanceof String) ? (String) authenticationInfo.get(ResourceResolverFactory.SUBSERVICE) : null;
                        ...
                        if (repo == null) {
                            log.warn("getResourceProviderInternal: Cannot login service because cannot get SlingRepository on behalf of bundle {} ({})", bc.getBundle().getSymbolicName(), bc.getBundle().getBundleId());
                            // TODO: correct ??
     ...
     */
    private static Session handleImpersonation(final Session session, final Map<String, Object> authenticationInfo, final boolean logoutSession) throws LoginException {
