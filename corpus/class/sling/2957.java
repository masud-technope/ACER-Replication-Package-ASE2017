...
public class ServletResourceProviderFactory {
    ...
    private String getPrefix(final ServiceReference ref) {
        ...
        if (value instanceof Number) {
            ...
            if (!s.startsWith("/")) {
                ...
                if (!isNumber) {
                    if (log.isDebugEnabled()) {
                        log.debug("getPrefix({}): Configuration property is ignored {}", getServiceIdentifier(ref), value);
                    }
                    if (this.servletRoot != null) {
                        return this.servletRoot;
                    }
    ...
    private void addByType(Set<String> pathSet, ServiceReference ref) {
        ...
        if (types == null || types.length == 0) {
            if (log.isDebugEnabled()) {
                log.debug("addByType({}): no resource types declared", getServiceIdentifier(ref));
            }
            return;
        }
        // check for selectors
        ...
        if (methods == null || methods.length == 0) {
            ...
            if (extensions == null || extensions.length == 0) {
                if (log.isDebugEnabled()) {
                    log.debug("addByType({}): No methods declared, assuming GET/HEAD", getServiceIdentifier(ref));
                }
                methods = DEFAULT_SERVLET_METHODS;
            }
        } else if (methods.length == 1 && ALL_METHODS.equals(methods[0])) {
            if (log.isDebugEnabled()) {
                log.debug("addByType({}): Assuming all methods for '*'", getServiceIdentifier(ref));
            }
            methods = null;
        }
