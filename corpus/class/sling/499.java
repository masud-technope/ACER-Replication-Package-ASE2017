...
@Service
@Reference(name = "feature", cardinality = ReferenceCardinality.OPTIONAL_MULTIPLE, policy = ReferencePolicy.DYNAMIC, referenceInterface = Feature.class)
@Properties({ @Property(name = "felix.webconsole.label", value = "features"), @Property(name = "felix.webconsole.title", value = "Features"), @Property(name = "felix.webconsole.category", value = "Sling"), @Property(name = "pattern", value = "/.*"), @Property(name = "service.ranking", intValue = 0x4000) })
        ...
        if ("GET".equals(((HttpServletRequest) req).getMethod())) {
            ...
            if (features == null || features.length == 0) {
                ...
                for (final Feature feature : features) {
                    pw.printf("<tr><td>%s</td><td>%s</td><td>%s</td></tr>%n", ResponseUtil.escapeXml(feature.getName()), ResponseUtil.escapeXml(feature.getDescription()), ctx.isEnabled(feature));
                }
                pw.println("</table>");
            }
        } else {
