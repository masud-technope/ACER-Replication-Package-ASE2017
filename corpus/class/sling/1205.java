...
public class CompositeHealthCheckTest {
    ...
    private HealthCheckExecutionResult createExecutionResult(String name, String[] testTags, Result result) {
        HealthCheckExecutionResult healthCheckExecutionResult = new ExecutionResult(new HealthCheckMetadata(new DummyHcServiceReference(name, testTags, new String[0])), result, 0L);
    ...
    public void testSimpleRecursion() {
        ...
        compositeHealthCheck.setHealthCheckFilter(new // not using @Spy because varargs matcher does not work with spies
        HealthCheckFilter(// not using @Spy because varargs matcher does not work with spies
        null) {
    ...
    public void testCyclicRecursion() {
        ...
        compositeHealthCheck.setHealthCheckFilter(new HealthCheckFilter(null) {
            ...
            @Override
            public ServiceReference[] getTaggedHealthCheckServiceReferences(String... tags) {
                // not using @Spy because varargs matcher does not work with spies
                ServiceReference[] result = new ServiceReference[] {};
                if (tags.length > 0) {
                    if (tags[0].equals(filterTags[0])) {
