...
public class Loader implements NamespaceMapper, BundleListener {
    ...
    private void registerNamespaces(Bundle bundle) {
        ...
        if (definition != null) {
            log.debug("registerNamespaces: Bundle {} tries to register: {}", bundle.getSymbolicName(), definition);
            ...
            while (st.hasMoreTokens()) {
                ...
                if (pos == -1) {
                    log.warn("registerNamespaces: Bundle {} has an invalid namespace manifest header entry: {}", bundle.getSymbolicName(), token);
                } else {
    ...
    private boolean registerBundleInternal(Bundle bundle, boolean isRetry) {
        try {
            ...
            if (isRetry) {
                log.error("Cannot register node types for bundle {}: {}", bundle.getSymbolicName(), re);
            } else {
                log.debug("Retrying to register node types failed for bundle {}: {}", bundle.getSymbolicName(), re);
            }
        }
        return false;
    }
    ...
    private boolean registerNodeTypes(Bundle bundle, boolean isRetry) throws RepositoryException {
        ...
        if (typesHeader == null) {
            ...
            log.debug("registerNodeTypes: Bundle {} has no nodetypes", bundle.getSymbolicName());
            return true;
        }
        ...
        try {
            ...
            while (tokener.hasMoreTokens()) {
                ...
                try {
                    ...
                    if (isRetry) {
                        log.info("Retrying to register node types from {} in bundle {} succeeded.", new Object[] { nodeTypeFile, bundle.getSymbolicName() });
                    }
