...
public class PreparePackageMojo extends AbstractLaunchpadFrameworkMojo {
     ...
     */
    private void patchSlingProperties(final File dest, final Properties additionalProps) throws MojoExecutionException {
     ...
     */
    private void patchSlingBootstrap(final File dest, final String additionalCmd) throws MojoExecutionException {
    ...
    private void copyBaseArtifact() throws MojoExecutionException {
        ...
        if (artifact == null) {
            throw new MojoExecutionException(String.format("Project doesn't have a base dependency of groupId %s and artifactId %s", base.getGroupId(), base.getArtifactId()));
        }
        ...
        if (additionalProps != null || bootstrapCmd != null) {
            ...
            if (shouldCopy(artifact.getFile(), destinationFile)) {
                try {
                    getLog().info(String.format("Copying base artifact from %s to %s.", artifact.getFile(), destinationFile));
                    FileUtils.copyFile(artifact.getFile(), destinationFile);
                } catch (IOException e) {
                    throw new MojoExecutionException("Unable to copy base artifact.", e);
                }
            } else {
                getLog().debug(String.format("Skipping copy of base artifact from %s.", artifact.getFile()));
            }
        }
    }
    ...
    private Artifact getBaseArtifact() throws MojoExecutionException {
        ...
        return getArtifact(base.getGroupId(), base.getArtifactId(), baseDependency.getVersion(), base.getType(), base.getClassifier());
    }

    private Artifact getBaseDependency() {
        return project.getArtifactMap().get(base.getGroupId() + ":" + base.getArtifactId());
    }
