...
public class InstallerConfig {
    ...
    public  InstallerConfig(final Logger logger, final ComponentContext ctx, final Dictionary<?, ?> cfg, final SlingSettingsService settings) {
        this.logger = logger;
        ...
        this.folderNameFilter = new FolderNameFilter(PropertiesUtil.toStringArray(getPropertyValue(logger, ctx, cfg, JcrInstaller.PROP_SEARCH_PATH), JcrInstaller.DEFAULT_SEARCH_PATH), folderNameRegexp, settings.getRunModes());
    ...
    private Object getPropertyValue(final Logger logger, final ComponentContext ctx, final Dictionary<?, ?> oldConfig, final String name) {
        ...
        if (oldConfig != null) {
            ...
            if (result != null) {
                logger.warn("Using configuration value from obsolete configuration with PID {} for property {}." + " Please merge this configuration into the configuration with the PID {}.", new Object[] { JcrInstaller.OLD_PID, name, ctx.getProperties().get(Constants.SERVICE_PID) });
            }
        }
        if (result == null) {
