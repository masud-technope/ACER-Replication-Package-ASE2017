...
public class BundleSnapshotUpdateTest extends OsgiInstallerTestBase {
    ...
    public void testNonSnapshot() throws Exception {
        ...
        installer.updateResources(URL_SCHEME, getInstallableResource(getTestBundle(BUNDLE_BASE_NAME + "-testbundle-1.1.jar"), "digest1"), null);
        // wait for two tasks: install and start
        this.waitForBundleEvents("Test bundle 1.1 must be found after waitForInstallerAction", listener, new BundleEvent(symbolicName, "1.1", org.osgi.framework.BundleEvent.INSTALLED), new BundleEvent(symbolicName, "1.1", org.osgi.framework.BundleEvent.STARTED));
        ...
        listener = this.startObservingBundleEvents();
        installer.updateResources(URL_SCHEME, getInstallableResource(getTestBundle(BUNDLE_BASE_NAME + "-testbundle-1.1.jar"), "digest2"), null);
        sleep(100);
    ...
    private void testSnapshotPrimitive(boolean restartInstaller) throws Exception {
        ...
        // wait for two tasks: install and start
        this.waitForBundleEvents("Test bundle must be found after waitForInstallerAction", listener, new BundleEvent(symbolicName, org.osgi.framework.BundleEvent.INSTALLED), new BundleEvent(symbolicName, org.osgi.framework.BundleEvent.STARTED));
        ...
        listener = this.startObservingBundleEvents();
        installer.updateResources(URL_SCHEME, getInstallableResource(getTestBundle(BUNDLE_BASE_NAME + "-snap.jar"), "digest2"), null);
        sleep(100);
        this.waitForBundleEvents("Test bundle must be updated with different digest and snapshot", listener, new BundleEvent(symbolicName, org.osgi.framework.BundleEvent.STOPPED), new BundleEvent(symbolicName, org.osgi.framework.BundleEvent.UPDATED), new BundleEvent(symbolicName, org.osgi.framework.BundleEvent.STARTED));
    }
