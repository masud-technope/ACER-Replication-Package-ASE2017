...
public class LogConfigManager implements LogbackResetListener, LogConfig.LogWriterProvider {
    ...
    public void updateLogWriter(String pid, Dictionary<?, ?> configuration, boolean performRefresh) throws ConfigurationException {
        if (configuration != null) {
            ...
            if (logFileName != null && !isConsole(logFileName)) {
                ...
                if (existingWriterByFileName != null && (oldWriter != null && !existingWriterByFileName.getConfigurationPID().equals(pid))) {
                    // this file is already configured by another LOG_PID
                    throw new ConfigurationException(LogConfigManager.LOG_FILE, "LogFile " + logFileName + " already configured by configuration " + existingWriterByFileName.getConfigurationPID());
                }
            }
            ...
            LogWriter newWriter = new LogWriter(pid, getAppnderName(logFileName), fileNum, fileSize, logFileName, bufferedLogging);
            if (oldWriter != null) {
        ...
        if (configuration != null) {
            ...
            for (final String cat : categories) {
                ...
                if (cfg != null && !pid.equals(cfg.getConfigPid())) {
                    throw new ConfigurationException(LogConfigManager.LOG_LOGGERS, "Category " + cat + " already defined by configuration " + pid);
                }
            }
            // verify log level
            if (level == null) {
    ...
    private Dictionary<String, String> getBundleConfiguration(BundleContext bundleContext) {
        ...
        final String[] props = { LOG_LEVEL, LOG_FILE, LOG_FILE_NUMBER, LOG_FILE_SIZE, LOG_PATTERN, LOGBACK_FILE, LOG_PACKAGING_DATA };
    ...
    private void processGlobalConfig(Dictionary<String, String> configuration) {
        ...
        maxCallerDataDepth = Util.toInteger(configuration.get(LOG_MAX_CLALLER_DEPTH), ClassicConstants.DEFAULT_MAX_CALLEDER_DATA_DEPTH);
        maxOldFileCount = Util.toInteger(configuration.get(PRINTER_MAX_INCLUDED_FILES), PRINTER_MAX_INCLUDED_FILES_DEFAULT);
        numOfLines = Util.toInteger(configuration.get(PRINTER_NUM_OF_LINES), PRINTER_NUM_OF_LINES_DEFAULT);
    }
