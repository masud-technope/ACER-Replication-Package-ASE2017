...
public class HeartbeatHandler extends BaseViewChecker {
     ...
     * voting.
     */
    public void initialize(final DiscoveryServiceImpl discoveryService, final String initialVotingId) {
        synchronized (lock) {
        ...
        try {
            ...
            final long maxMillisSinceHb = Math.max(Math.min(heartbeatTimeoutMillis, 2 * heartbeatIntervalMillis), heartbeatTimeoutMillis - 2 * heartbeatIntervalMillis);
                ...
                public void run() {
                    ...
                    if (lastHb != null) {
                        ...
                        if (timeSinceHb > maxMillisSinceHb) {
                            logger.warn("checkForLocalClusterViewChange/.run: time since local instance last wrote a heartbeat is " + timeSinceHb + "ms" + " (heartbeatTimeoutMillis=" + heartbeatTimeoutMillis + ", heartbeatIntervalMillis=" + heartbeatIntervalMillis + " => maxMillisSinceHb=" + maxMillisSinceHb + "). Flagging us as (still) changing");
    ...
    private String newLeaderElectionId(ResourceResolver resourceResolver) {
        ...
        if (leaderElectionRepositoryDescriptor != null && leaderElectionRepositoryDescriptor.length() != 0) {
            ...
            if (session != null) {
                String value = session.getRepository().getDescriptor(leaderElectionRepositoryDescriptor);
                if (value != null) {
        ...
        final String newLeaderElectionId = prefix + "_" + currentTimeMillisStr + "_" + slingId;
    ...
    private void doStartNewVoting(final ResourceResolver resourceResolver, final Set<String> liveInstances) throws PersistenceException {
    ...
    public void invalidateCurrentEstablishedView() {
        ...
        logger.info("invalidateCurrentEstablishedView: invalidating slingId=" + slingId + ", lastEstablishedViewId=" + lastEstablishedViewId);
