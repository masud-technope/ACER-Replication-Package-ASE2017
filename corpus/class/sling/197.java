...
public class SuspendableOutputStream extends FilterOutputStream implements JobStatus {
    ...
    public synchronized void requestStateChange(State s) {
        ...
        // ignore changes
            ...
            if (state == State.NEW || state == State.QUEUED || state == State.RUNNING || state == State.SUSPEND_REQUESTED || state == State.SUSPENDED) {
            ...
            if (state == State.QUEUED || state == State.SUSPEND_REQUESTED || state == State.SUSPENDED) {
                state = State.RUNNING;
                notify();
            }
        } else {
