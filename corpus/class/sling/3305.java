...
public class AttachBundleListMojo extends AbstractUsingBundleListMojo {
    ...
    private void attachConfigurations() throws MojoExecutionException, IOException, ArchiverException {
        ...
        if (this.getSlingBootstrap(false) != null) {
            ...
            slingDir.mkdirs();
            FileUtils.fileWrite(new File(slingDir, AttachPartialBundleListMojo.SLING_WEBAPP_BOOTSTRAP).getAbsolutePath(), "UTF-8", this.getSlingBootstrap(false));
        }
        ...
        if (this.getSlingBootstrap(true) != null) {
            final File slingDir = new File(this.configOutputDir, "sling");
            slingDir.mkdirs();
            FileUtils.fileWrite(new File(slingDir, AttachPartialBundleListMojo.SLING_STANDALONE_BOOTSTRAP).getAbsolutePath(), "UTF-8", this.getSlingBootstrap(true));
        }
        ...
        if (this.checkFile(this.getConfigDirectory())) {
            ...
            configDir.mkdirs();
            copyDirectory(this.getConfigDirectory(), configDir, null, FileUtils.getDefaultExcludes());
        }
        ...
        projectHelper.attachArtifact(project, AttachPartialBundleListMojo.CONFIG_TYPE, AttachPartialBundleListMojo.CONFIG_CLASSIFIER, destFile);
    }
}
