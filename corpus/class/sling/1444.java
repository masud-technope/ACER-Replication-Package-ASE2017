...
public class VelocityTemplatesScriptEngine extends AbstractSlingScriptEngine {
    ...
    public  VelocityTemplatesScriptEngine(ScriptEngineFactory factory) {
        ...
        try {
            ...
            throw new RuntimeException("Exception in Velocity.init() " + e.getMessage(), e);
        } finally {
    ...
    public Object eval(Reader script, ScriptContext scriptContext) throws ScriptException {
        if (script == null) {
            throw new IllegalArgumentException("Method argument script must not be null.");
        }
        if (scriptContext == null) {
            throw new IllegalArgumentException("Method argument scriptContext must not be null.");
        }
        ...
        try {
            ...
            try {
                ...
                throw new ScriptException("Failure running script " + scriptName + ": " + t + ", stack trace: " + t.getStackTrace());
            }
        } finally {
            Thread.currentThread().setContextClassLoader(oldClassLoader);
        }
