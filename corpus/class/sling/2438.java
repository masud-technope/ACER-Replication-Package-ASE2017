...
public class AddOrUpdateNodeCommandTest {
    ...
    private void doPropertyChangeTest(final Object initialPropertyValues, final Object newPropertyValues) throws Exception {
    ...
    public void autoCreatedPropertiesAreNotRemoved() throws Exception {
        doWithTransientRepository(new CallableWithSession() {
            ...
            public Void call() throws Exception {
                ...
                AddOrUpdateNodeCommand cmd = new AddOrUpdateNodeCommand(repo(), credentials(), DEFAULT_CONTEXT, null, resource, logger);
                cmd.execute().get();
                // second time since mixins are processed after properties so we need two
                cmd.execute().get();
                // executions to
                // expose the problem
                session().refresh(false);
    ...
    private void importNodeTypeDefinitions(Session session, String cndFile) throws InvalidNodeTypeDefinitionException, NodeTypeExistsException, UnsupportedRepositoryOperationException, ParseException, RepositoryException, IOException {
    ...
    private abstract static class CallableWithSession implements Callable<Void> {
