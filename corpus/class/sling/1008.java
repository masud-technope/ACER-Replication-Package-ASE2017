...
public class VotingHelper {
     ...
     */
    public static List<VotingView> listOpenNonWinningVotings(final ResourceResolver resourceResolver, final Config config) {
        if (config == null) {
        ...
        final Resource ongoingVotingsResource = resourceResolver.getResource(ongoingVotingsPath);
        ...
        while (it.hasNext()) {
            ...
            if (matchesLiveView == null && ongoingVoting && !hasNoVotes && !isWinning) {
                if (logger.isDebugEnabled()) {
                    logger.debug("listOpenNonWinningVotings: found an open voting: " + aChild + ", properties=" + ResourceHelper.getPropertiesForLogging(aChild));
                }
                result.add(c);
            } else {
                if (logger.isDebugEnabled()) {
                    logger.debug("listOpenNonWinningVotings: a non-open voting: " + aChild + ", matches live: " + matchesLiveView + ", is ongoing: " + ongoingVoting + ", has no votes: " + hasNoVotes + ", is winning: " + isWinning + ", properties=" + ResourceHelper.getPropertiesForLogging(aChild));
                }
            }
        }
        if (logger.isDebugEnabled()) {
     ...
     */
    public static List<VotingView> listTimedoutVotings(final ResourceResolver resourceResolver, final Config config) {
        final String ongoingVotingsPath = config.getOngoingVotingsPath();
        final Resource ongoingVotingsResource = resourceResolver.getResource(ongoingVotingsPath);
        if (ongoingVotingsResource == null) {
     ...
     */
    public static VotingView getWinningVoting(final ResourceResolver resourceResolver, final Config config) {
     ...
     * initiator
     */
    public static List<VotingView> getYesVotingsOf(final ResourceResolver resourceResolver, final Config config, final String slingId) {
