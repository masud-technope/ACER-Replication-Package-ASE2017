...
@Component(metatype = true, label = "Apache Sling Script Cache", description = "The Script Cache is useful for running previously compiled scripts.")
@Properties({ @Property(name = ScriptCacheImpl.PROP_CACHE_SIZE, intValue = ScriptCacheImpl.DEFAULT_CACHE_SIZE, label = "Cache Size", description = "The Cache Size defines the maximum number of compiled script references that will be stored in the cache's" + " internal map."), @Property(name = ScriptCacheImpl.PROP_ADDITIONAL_EXTENSIONS, value = "", label = "Additional Extensions", description = "Scripts from the search paths with these extensions will also be monitored so that changes to them will " + "clean the cache if the cache contains them.", unbounded = PropertyUnbounded.ARRAY) })
@Service(ScriptCache.class)
@Reference(name = "scriptEngineFactory", cardinality = ReferenceCardinality.OPTIONAL_MULTIPLE, referenceInterface = ScriptEngineFactory.class, policy = ReferencePolicy.DYNAMIC)
@SuppressWarnings("unused")
public class ScriptCacheImpl implements /**
    ...
    private void configureCache() {
        ...
        try {
            ...
            if (extensions.size() > 0) {
                ...
                eventHandlerProperties.put(EventConstants.EVENT_TOPIC, new String[] { SlingConstants.TOPIC_RESOURCE_CHANGED, SlingConstants.TOPIC_RESOURCE_REMOVED });
