...
public class SlingLogPanel extends HttpServlet {
    ...
    private static final String[] LEVEL_NAMES = { Level.ERROR.levelStr, Level.WARN.levelStr, Level.INFO.levelStr, Level.DEBUG.levelStr, Level.TRACE.levelStr, Level.OFF.levelStr, LogConfigManager.LOG_LEVEL_RESET_TO_DEFAULT };
    ...
    protected URL getResource(String path) {
        return //
        (path != null && path.startsWith(labelRes)) ? getClass().getResource(path.substring(labelResLen)) : null;
    }
    ...
    private void renderAppenderContent(LoggerStateContext ctx, PrintWriter pw, String appenderName, int numOfLines) throws IOException {
        for (final Appender<ILoggingEvent> appender : ctx.appenders.values()) {
    ...
    private String getLinkedName(FileAppender<ILoggingEvent> appender) throws UnsupportedEncodingException {
        ...
        return String.format("File : [<a href=\"%s/%s?%s=%d&%s=%s\">%s</a>] %s", APP_ROOT, PATH_TAILER, PARAM_NUM_OF_LINES, logbackManager.getLogConfigManager().getNumOfLines(), PARAM_APPENDER_NAME, URLEncoder.encode(name, "UTF-8"), XmlUtil.escapeXml(name), XmlUtil.escapeXml(fileName));
    }
     ...
     */
    private void configureLogger(final String pid, final String logLevel, final String[] loggers, final String logFile, String additive) throws IOException, ConfigurationException {
                ...
                if (configAdmin != null) {
                    try {
                        ...
                        if (config != null) {
                            ...
                            throw new ConfigurationException(LogConfigManager.PID, "No configuration for this PID:" + pid);
                        }
    ...
    private static String formatPid(final String consoleAppRoot, final TurboFilter tf, final LoggerStateContext ctx) {
    ...
    private static String formatPid(final String consoleAppRoot, final Appender<ILoggingEvent> appender, final LoggerStateContext ctx) {
