...
public class JCRURLConnection extends URLConnection {
        ...
        if (!connected) {
            ...
            try {
                ...
                if (parent.hasProperty("jcr:mimeType")) {
                    ...
                    if (contentType == null) {
                        contentType = (property.getType() == PropertyType.BINARY) ? APPLICATION_OCTET : TEXT_PLAIN;
                    }
                }
                if (parent.hasProperty("jcr:encoding")) {
                    contentEncoding = parent.getProperty("jcr:encoding").getString();
                }
                log.debug("connect: Using property '{}' with content type '{}' for {} bytes", new Object[] { property.getPath(), contentType, new Integer(contentLength) });
                // set the fields
    ...
    private IOException failure(String method, String message, Throwable cause) {
        log.info(method + ": URL: " + url.toExternalForm() + ", Reason: " + message);
        if (cause != null) {
