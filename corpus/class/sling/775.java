...
@Service
@org.apache.felix.scr.annotations.Properties({ @Property(name = Constants.SERVICE_VENDOR, value = "The Apache Software Foundation"), @Property(name = Constants.SERVICE_DESCRIPTION, value = "Apache Sling OpenID Authentication Handler"), @Property(name = AuthenticationHandler.PATH_PROPERTY, value = "/", unbounded = PropertyUnbounded.ARRAY), @Property(name = AuthenticationHandler.TYPE_PROPERTY, value = OpenIDConstants.OPENID_AUTH, propertyPrivate = true) })
public class OpenIDAuthenticationHandler extends AbstractAuthenticationHandler {
        ...
        try {
            ...
            if (user == null) {
                ...
                if (RelyingParty.isAuthResponse(request)) {
                    ...
                    // failed verification
                    return handleAuthFailure(OpenIDFailure.VERIFICATION, request);
                }
                ...
                if (relyingParty.associateAndAuthenticate(user, request, response, trustRoot, trustRoot, returnTo)) {
    ...
    public boolean requestCredentials(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        params.put(Authenticator.LOGIN_RESOURCE, getLoginResource(request, null));
        ...
        if (paramIdentifier instanceof String) {
            params.put(OpenIDConstants.OPENID_IDENTITY, (String) paramIdentifier);
        }
        try {
    ...
    public void authenticationFailed(HttpServletRequest request, HttpServletResponse response, AuthenticationInfo authInfo) {
        ...
        dropCredentials(request, response);
        request.setAttribute(OpenIDConstants.OPENID_FAILURE_REASON, OpenIDFailure.REPOSITORY);
        if (user != null && user.getIdentity() != null) {
            request.setAttribute(OpenIDConstants.OPENID_IDENTITY, user.getIdentity());
        }
    }
     ...
     */
    private OpenIdUser discover(final RelyingParty relyingParty, final HttpServletRequest request) {
        try {
            // OpenIdUser instance created by the old bundle.
            return relyingParty.discover(request);
        } catch (UnknownHostException uhe) {
            log.info("discover: The OpenID identifier cannot be resolved because it designates an unknown host {}", uhe.getMessage());
        } catch (IOException ioe) {
            log.info("discover: Failure to communicate with OpenID provider", ioe);
        } catch (ClassCastException cce) {
            throw cce;
        } catch (Exception e) {
            log.warn("discover: Unexpected failure discovering the OpenID user", e);
        }
    ...
    private AuthenticationInfo handleAuthFailure(OpenIDFailure failure, HttpServletRequest request) {
        request.setAttribute(OpenIDConstants.OPENID_FAILURE_REASON, failure);
    ...
    protected void activate(ComponentContext componentContext) {
        ...
        loginForm = OsgiUtil.toString(props.get(PROP_LOGIN_FORM), AuthenticationFormServlet.SERVLET_PATH);
        externalUrlPrefix = OsgiUtil.toString(props.get(PROP_EXTERNAL_URL_PREFIX), DEFAULT_EXTERNAL_URL_PREFIX);
        // JCR user properties used to match OpenID users
        identityProperty = OsgiUtil.toString(props.get(PROP_OPEN_ID_IDENTIFIER_PROPERTY), DEFAULT_OPEN_ID_IDENTIFIER_PROPERTY);
        // DYU OpenID properties
        useCookie = OsgiUtil.toBoolean(props.get(PROP_USE_COOKIE), DEFAULT_USE_COOKIE);
        cookieDomain = OsgiUtil.toString(props.get(PROP_COOKIE_DOMAIN), DEFAULT_COOKIE_DOMAIN);
        cookieName = OsgiUtil.toString(props.get(PROP_COOKIE_NAME), DEFAULT_COOKIE_NAME);
        identifierParam = OsgiUtil.toString(props.get(PROP_LOGIN_IDENTIFIER_FORM_FIELD), DEFAULT_LOGIN_IDENTIFIER_FORM_FIELD);
        cookieSecret = OsgiUtil.toString(props.get(PROP_COOKIE_SECRET_KEY), DEFAULT_COOKIE_SECRET_KEY).toCharArray();
        openIdAttribute = OsgiUtil.toString(props.get(PROP_OPENID_USER_ATTR), DEFAULT_OPENID_USER_ATTR);
    ...
    protected void deactivate(@SuppressWarnings("unused") ComponentContext componentContext) {
        ...
        if (session != null) {
            try {
                ...
                log.error("deactivate: Unexpected problem logging out session", t);
            }
            userManager = null;
            session = null;
        }
    ...
    private boolean ignoreRequestCredentials(final HttpServletRequest request) {
        ...
        return requestLogin != null && !OpenIDConstants.OPENID_AUTH.equals(requestLogin);
    }

    private AuthenticationInfo getAuthInfoFromUser(final OpenIdUser user) {
        final AuthenticationInfo info = new AuthenticationInfo(OpenIDConstants.OPENID_AUTH, getUserName(user));
    ...
    private String getUserIdByProperty(final UserManager userManager, final String propName, final String propValue) {
        String userId = null;
        try {
            Iterator<?> users = userManager.findAuthorizables(propName, propValue, UserManager.SEARCH_TYPE_USER);
            // use the first user found
            if (users.hasNext()) {
                userId = ((User) users.next()).getID();
                // warn if more than one user found
                if (users.hasNext()) {
                    log.warn("getUserName: Multiple users found with property {}={}; using {}", new Object[] { propName, propValue, userId });
                }
            }
        } catch (RepositoryException re) {
            log.warn("getUserName: Problem finding user with property {}={}", new Object[] { propName, propValue }, re);
        }
        return userId;
    }
    ...
    private RelyingParty getRelyingParty(final HttpServletRequest request) {
        if (relyingParty == null) {
            ...
            if (useCookie) {
                ...
                openIdProps.setProperty("openid.user.manager.cookie.security.secret_key", new String(cookieSecret));
            }
