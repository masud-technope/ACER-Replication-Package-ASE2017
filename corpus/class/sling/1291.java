...
public class JSONResponseTest extends ServerSideTestsBase {
    ...
    private void testJsonListOfTests() throws Exception {
        Request r = getRequestBuilder().buildGetRequest(JUNIT_SERVLET_PATH + "/.json").withCredentials(getServerUsername(), getServerPassword());
        // Get list of tests in JSON format
        getRequestExecutor().execute(r).assertStatus(200).assertContentType("application/json");
        ...
        final List<String> expectedTestNames = Arrays.asList(new String[] { "org.apache.sling.testing.samples.failingtests.EmptyTest", "org.apache.sling.testing.samples.failingtests.JUnit3FailingTest", "org.apache.sling.testing.samples.failingtests.JUnit4FailingTest", "org.apache.sling.testing.samples.sampletests.JUnit3Test", "org.apache.sling.testing.samples.sampletests.JUnit4Test", "org.apache.sling.testing.samples.sampletests.OsgiAwareTest", "org.apache.sling.junit.scriptable.ScriptableTestsProvider" });
        for (String name : expectedTestNames) {
            assertTrue("Expecting test name " + name + " in json response", findTestName(json, name));
        }
    }
}
