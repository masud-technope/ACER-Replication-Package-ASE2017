...
public class InstallEditorSection extends ServerEditorSection {
    ...
    public void init(IEditorSite site, IEditorInput input) {
        ...
        serverListener = new PropertyChangeListener() {
            ...
            public void propertyChange(PropertyChangeEvent evt) {
                ...
                } else if (evt.getPropertyName().equals(String.format(ISlingLaunchpadServer.PROP_BUNDLE_VERSION_FORMAT, EmbeddedArtifactLocator.SUPPORT_BUNDLE_SYMBOLIC_NAME))) {
                ...
                try {
                    dialog.run(true, false, new IRunnableWithProgress() {
                        ...
                        public void run(IProgressMonitor monitor) throws InvocationTargetException, InterruptedException {
                            ...
                            try {
                                RepositoryInfo repositoryInfo = ServerUtil.getRepositoryInfo(server.getOriginal(), monitor);
                                OsgiClient client = osgiClientFactory.createOsgiClient(repositoryInfo);
                                remoteVersion = client.getBundleVersion(EmbeddedArtifactLocator.SUPPORT_BUNDLE_SYMBOLIC_NAME);
                                ...
                                Display.getDefault().syncExec(new Runnable() {
                                    ...
                                    @Override
                                    public void run() {
                                        execute(new SetBundleVersionCommand(server, EmbeddedArtifactLocator.SUPPORT_BUNDLE_SYMBOLIC_NAME, finalDeployedVersion.toString()));
                                        try {
                            ...
                            Display.getDefault().asyncExec(new Runnable() {
                                ...
                                @Override
                                public void run() {
                                    MessageDialog.openInformation(getShell(), "Support bundle install operation", message);
                                }
                            });
                        }
                    });
                } catch (InvocationTargetException e1) {
                    IStatus status = new Status(Status.ERROR, Activator.PLUGIN_ID, "Error while installing support bundle: " + e1.getTargetException().getMessage(), e1.getTargetException());
    ...
    private void updateActionArea(Version serverVersion, final Version embeddedVersion) {
        if (serverVersion == null || embeddedVersion.compareTo(serverVersion) > 0) {
            supportBundleVersionLabel.setText("Installation support bundle is not present or outdated, deployment will not work");
