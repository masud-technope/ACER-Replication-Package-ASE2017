...
public class ChunkCleanUpTask implements Runnable {
    ...
    private void cleanup() {
        ...
        try {
            ...
            QueryResult queryres = qm.createQuery("SELECT * FROM [sling:chunks] ", Query.JCR_SQL2).execute();
        ...
        log.info("ChunkCleanUpTask finished: Removed {} chunk upload(s) in {}ms ({} chunk upload(s) still active)", new Object[] { numCleaned, (end - start), numLive });
    }
    ...
    private boolean isEligibleForCleanUp(Node node) throws RepositoryException {
        ...
        return lastChunkNode != null && (System.currentTimeMillis() - lastChunkNode.getProperty(javax.jcr.Property.JCR_CREATED).getDate().getTimeInMillis()) > chunkCleanUpAge;
    }

    @Activate
    protected void activate(final ComponentContext context, final Map<String, Object> configuration) {
        chunkCleanUpAge = OsgiUtil.toInteger(configuration.get(CHUNK_CLEANUP_AGE), 1) * 60 * 1000;
        log.info("scheduler config [{}], chunkGarbageTime  [{}] ms", OsgiUtil.toString(configuration.get("scheduler.expression"), ""), chunkCleanUpAge);
    }
}
