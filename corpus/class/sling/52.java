...
public class SetupService {
    ...
    private static final String[] FOLDERS = new String[] { "content:" + SlingshotConstants.RESOURCETYPE_CONTENT, "info", "profile", "ugc" };
    ...
    private void setupACL(final ResourceResolver resolver) throws RepositoryException {
        ...
        for (final String principalId : USERS) {
            ...
            modifyAce(session, ugcPath, InternalConstants.SERVICE_USER_NAME, Privilege.JCR_ALL, true);
        }
    }

    private void modifyAce(final Session jcrSession, final String resourcePath, final String principalId, final String privilege, final boolean granted) throws RepositoryException {
        ...
        AccessControlUtil.replaceAccessControlEntry(jcrSession, resourcePath, principal, grantedPrivilegeNames, deniedPrivilegeNames, null, null);
    ...
    private void setupUsers(final BundleContext bc, final ResourceResolver resolver) throws RepositoryException, IOException {
        ...
        try {
            final Configuration[] configs = this.configAdmin.listConfigurations("(&(" + ConfigurationAdmin.SERVICE_FACTORYPID + "=org.apache.sling.serviceusermapping.impl.ServiceUserMapperImpl.amended" + ")(user.mapping=" + bc.getBundle().getSymbolicName() + "*" + "))");
