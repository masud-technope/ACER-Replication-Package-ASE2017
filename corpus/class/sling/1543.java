...
public class QuartzJobExecutor implements Job {
    ...
    public void execute(final JobExecutionContext context) throws JobExecutionException {
        ...
        if (runOn != null) {
            if (runOn.length == 1 && (Scheduler.VALUE_RUN_ON_LEADER.equals(runOn[0]) || Scheduler.VALUE_RUN_ON_SINGLE.equals(runOn[0]))) {
                if (DISCOVERY_AVAILABLE.get()) {
                    if (DISCOVERY_INFO_AVAILABLE.get()) {
                        if (!IS_LEADER.get()) {
                            logger.debug("Excluding job {} with name {} and config {} - instance is not leader", new Object[] { job, data.get(QuartzScheduler.DATA_MAP_NAME), runOn[0] });
                            return;
                        }
                    } else {
                        logger.debug("No discovery info available. Excluding job {} with name {} and config {}.", new Object[] { job, data.get(QuartzScheduler.DATA_MAP_NAME), runOn[0] });
                        return;
                    }
                } else {
                    logger.error("No discovery available, therefore not executing job {} with name {} and config {}.", new Object[] { job, data.get(QuartzScheduler.DATA_MAP_NAME), runOn[0] });
                    return;
                }
            } else {
                // sling IDs
                ...
                if (myId == null) {
                    logger.error("No Sling ID available, therefore not executing job {} with name {} and config {}.", new Object[] { job, data.get(QuartzScheduler.DATA_MAP_NAME), Arrays.toString(runOn) });
                    return;
                } else {
                    ...
                    if (!schedule) {
                        logger.debug("Excluding job {} with name {} and config {} - different Sling ID", new Object[] { job, data.get(QuartzScheduler.DATA_MAP_NAME), Arrays.toString(runOn) });
                        return;
                    }
                }
            }
