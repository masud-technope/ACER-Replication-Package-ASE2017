...
public abstract class LogTestBase {
    ...
    private static Option addDebugOptions() throws IOException {
        if (paxRunnerVmOption != null) {
            ...
            return composite(CoreOptions.vmOption(paxRunnerVmOption), keepCaches(), systemTimeout(TimeUnit.MINUTES.toMillis(10)), workingDirectory(workDir));
        }
        return null;
    }
    ...
    protected static Option webSupport() {
        return composite(mavenBundle("org.apache.felix", "org.apache.felix.http.bundle").versionAsInProject(), systemProperty("org.osgi.service.http.port").value(getServerPort()));
    }
