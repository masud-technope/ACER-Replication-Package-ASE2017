...
public class ConfigTaskCreator implements InstallTaskFactory, ConfigurationListener, ResourceTransformer {
    ...
    public void configurationEvent(final ConfigurationEvent event) {
        synchronized (Coordinator.SHARED) {
            ...
            if (event.getType() == ConfigurationEvent.CM_DELETED) {
                ...
                try {
                    final Configuration config = ConfigUtil.getConfiguration(configAdmin, event.getFactoryPid(), event.getPid());
    ...
    private static boolean isConfigExtension(String extension) {
        if (extension.equals("cfg") || extension.equals("config") || extension.equals("xml") || extension.equals("properties")) {
            return true;
        }
        return false;
    }
