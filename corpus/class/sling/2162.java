...
@Properties({ @Property(name = Constants.SERVICE_DESCRIPTION, value = "Apache Sling OSGi Installer Web Console Plugin"), @Property(name = "felix.webconsole.label", value = "osgi-installer"), @Property(name = "felix.webconsole.title", value = "OSGi Installer"), @Property(name = "felix.webconsole.category", value = "OSGi"), @Property(name = "felix.webconsole.configprinter.modes", value = { "zip", "txt" }) })
...
public class OsgiInstallerWebConsolePlugin extends GenericServlet {
    ...
    public void service(final ServletRequest req, final ServletResponse res) throws IOException {
        ...
        for (final ResourceGroup group : state.getActiveResources()) {
            ...
            pw.printf("<tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td></tr>", getEntityId(toActivate, group.getAlias()), getInfo(toActivate), getURL(toActivate), toActivate.getState());
        }
        if (rt != null) {
            pw.println("</tbody></table>");
        }
        ...
        for (final ResourceGroup group : state.getInstalledResources()) {
            ...
            if (resources.size() > 0) {
                ...
                if (first.getAttribute(TaskResource.ATTR_INSTALL_EXCLUDED) != null) {
                    pw.printf("<tr><td></td><td colspan='2'>%s</td><td></td></tr>", first.getAttribute(TaskResource.ATTR_INSTALL_EXCLUDED));
                }
                if (first.getAttribute(TaskResource.ATTR_INSTALL_INFO) != null) {
                    pw.printf("<tr><td></td><td colspan='2'>%s</td><td></td></tr>", first.getAttribute(TaskResource.ATTR_INSTALL_INFO));
                }
                while (iter.hasNext()) {
                    final Resource resource = iter.next();
                    pw.printf("<tr><td></td><td>%s</td><td>%s</td><td>%s</td></tr>", getInfo(resource), getURL(resource), resource.getState());
                }
            }
        }
        if (rt != null) {
