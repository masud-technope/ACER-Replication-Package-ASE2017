...
public class SlingAuthenticator implements Authenticator, AuthenticationSupport, ServletRequestListener {
    ...
    // TODO: This should be a PASSWORD type
    @Property()
    ...
    private void activate(final BundleContext bundleContext, final Map<String, Object> properties) {
        ...
        serviceListener = SlingAuthenticatorServiceListener.createListener(bundleContext, this);
    ...
    private void modified(Map<String, Object> properties) {
        ...
        if (!newCookie.equals(this.sudoCookieName)) {
            log.info("modified: Setting new cookie name for impersonation {} (was {})", newCookie, this.sudoCookieName);
        ...
        if (!newPar.equals(this.sudoParameterName)) {
            log.info("modified: Setting new parameter name for impersonation {} (was {})", newPar, this.sudoParameterName);
        ...
        if (authReqs != null) {
            for (String authReq : authReqs) {
                if (authReq != null && authReq.length() > 0) {
                    authRequiredCache.addHolder(AuthenticationRequirementHolder.fromConfig(authReq, null));
                }
            }
        }
        ...
        authUriSuffices = OsgiUtil.toStringArray(properties.get(PAR_AUTH_URI_SUFFIX), new String[] { DEFAULT_AUTH_URI_SUFFIX });
        // don't require authentication for login/logout servlets
        authRequiredCache.addHolder(new AuthenticationRequirementHolder(LoginServlet.SERVLET_PATH, false, null));
        authRequiredCache.addHolder(new AuthenticationRequirementHolder(LogoutServlet.SERVLET_PATH, false, null));
    ...
    public void login(HttpServletRequest request, HttpServletResponse response) {
        ...
        final Collection<AbstractAuthenticationHandlerHolder>[] holdersArray = this.authHandlerCache.findApplicableHolder(request);
                ...
                for (AbstractAuthenticationHandlerHolder holder : holderList) {
                    if (path.startsWith(holder.path)) {
                        ...
                        try {
                            ...
                            log.error("login: Failed sending authentication request through handler " + holder + ", access forbidden", ioe);
                            done = true;
                        }
                        if (done) {
                            break;
    ...
    private AuthenticationInfo getAuthenticationInfo(HttpServletRequest request, HttpServletResponse response) {
        ...
        final Collection<AbstractAuthenticationHandlerHolder>[] localArray = this.authHandlerCache.findApplicableHolder(request);
     ...
     *         is terminated.
     */
    private boolean getResolver(final HttpServletRequest request, final HttpServletResponse response, final AuthenticationInfo authInfo) {
    ...
    private boolean isAnonAllowed(HttpServletRequest request) {
        ...
        final Collection<AuthenticationRequirementHolder>[] holderSetArray = authRequiredCache.findApplicableHolder(request);
    ...
    private void doLogin(HttpServletRequest request, HttpServletResponse response) {
        ...
        ensureAttribute(request, AuthenticationHandler.FAILURE_REASON, "Authentication Failed");
        AuthUtil.sendInvalid(request, response);
    }
     ...
     */
    private void setAttributes(final ResourceResolver resolver, final String authType, final HttpServletRequest request) {
     ...
     */
    private void sendSudoCookie(HttpServletResponse response, final String user, final int maxAge, final String path, final String owner) {
        final String quotedUser;
        try {
            quotedUser = quoteCookieValue(user);
        } catch (IllegalArgumentException iae) {
            log.error("sendSudoCookie: Failed to quote value '{}' of cookie {}: {}", new Object[] { user, this.sudoCookieName, iae.getMessage() });
            return;
        }
        if (quotedUser != null) {
    ...
    private static class SlingAuthenticatorServiceListener implements AllServiceListener {
        ...
        static SlingAuthenticatorServiceListener createListener(final BundleContext context, final SlingAuthenticator authenticator) {
            SlingAuthenticatorServiceListener listener = new SlingAuthenticatorServiceListener(authenticator);
            try {
        ...
        private  SlingAuthenticatorServiceListener(final SlingAuthenticator authenticator) {
            this.authenticator = authenticator;
        }

        public void serviceChanged(final ServiceEvent event) {
            synchronized (props) {
                if ((event.getType() & (ServiceEvent.MODIFIED | ServiceEvent.UNREGISTERING | 8)) != 0) {
        ...
        private void registerService(final AuthenticationRequirementHolder[] authReqs) {
    ...
    private static class AuthenticationHandlerTracker extends ServiceTracker {
        ...
        private final HashMap<Object, AbstractAuthenticationHandlerHolder[]> handlerMap = new HashMap<Object, AbstractAuthenticationHandlerHolder[]>();
         ...
         AuthenticationHandlerTracker(final BundleContext context, final PathBasedHolderCache<AbstractAuthenticationHandlerHolder> authHandlerCache) {
            this(context, AuthenticationHandler.SERVICE_NAME, authHandlerCache);
        }

        protected  AuthenticationHandlerTracker(final BundleContext context, final String className, final PathBasedHolderCache<AbstractAuthenticationHandlerHolder> authHandlerCache) {
        ...
        protected AbstractAuthenticationHandlerHolder createHolder(final String path, final Object handler, final ServiceReference serviceReference) {
    ...
    private static class EngineAuthenticationHandlerTracker extends AuthenticationHandlerTracker {
         ...
         EngineAuthenticationHandlerTracker(final BundleContext context, final PathBasedHolderCache<AbstractAuthenticationHandlerHolder> authHandlerCache) {
            super(context, "org.apache.sling.engine.auth.AuthenticationHandler", authHandlerCache);
        }

        @SuppressWarnings("deprecation")
        @Override
        protected AbstractAuthenticationHandlerHolder createHolder(String path, Object handler, final ServiceReference serviceReference) {
