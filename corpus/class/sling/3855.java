...
class BundleSpec {
     ...
     */
     BundleSpec(String bundleSpec) {
        ...
        if (!tokener.hasMoreTokens()) {
            throw new IllegalArgumentException("Illegal Bundle specification: " + bundleSpec);
        }
    ...
    public String toFilter() {
        ...
        return "(&(symbolicname=" + this.getSymbolicName() + ")" + this.getVersion().getFilter() + ")";
    }
    ...
    private String checkSymbolicName(String symbolicName) {
        if (symbolicName == null || symbolicName.length() == 0) {
            throw new IllegalArgumentException("Symbolic Name must not be empty");
        }
        if (!SYMBOLICNAME.matcher(symbolicName).matches()) {
            throw new IllegalArgumentException("Symbolic Name " + symbolicName + " contains illegal characters");
        }
        return symbolicName;
    }
