...
public class RegisteredResourceImpl implements TaskResource, Serializable, Comparable<RegisteredResourceImpl> {
	 ...
	 */
    private  RegisteredResourceImpl(final String id, final String resourceUri, final File file, final Dictionary<String, Object> dict, final String type, final String digest, final int priority, final String scheme) {
        this.url = scheme + ':' + id;
    ...
    private void update(final TransformationResult tr) throws IOException {
        ...
        if (tr.getResourceType() != null) {
            ...
            if (tr.getId() != null) {
                ...
                if (!InstallableResource.TYPE_FILE.equals(this.getType()) && !InstallableResource.TYPE_PROPERTIES.equals(this.getType())) {
    ...
    public TaskResource clone(TransformationResult transformationResult) throws IOException {
        ...
        final RegisteredResourceImpl rr = new RegisteredResourceImpl(this.url.substring(schemePos + 1), this.dataUri, this.dataFile, this.dictionary, this.resourceType, this.digest, this.priority, this.urlScheme);
        rr.attributes.putAll(this.attributes);
        rr.update(transformationResult);
        return rr;
    }

    public void update(final File file, final Dictionary<String, Object> dict, final String digest, final int priority, final String url) {
        this.removeDataFile();
        if (file != null) {
            this.dataFile = file;
        } else {
