...
public class SharedDataGraph implements ISharedObject, ISharedDataGraph {
    ...
    private Version version;
     ...
     SharedDataGraph(DataGraph dataGraph, IUpdateProvider updateProvider, IUpdateConsumer updateConsumer, IPublicationCallback publicationCallback, ISubscriptionCallback subscriptionCallback) {
    ...
    public synchronized void commit() throws ECFException {
        ...
        try {
            config.getContext().sendMessage(null, new UpdateDataGraphMessage(version, data));
        } catch (IOException e) {
     ...
     */
    public synchronized void init(ISharedObjectConfig initData) throws SharedObjectInitException {
        if (config == null)
            config = initData;
        else
    ...
    public void handleEvent(Event event) {
        if (event instanceof ISharedObjectActivatedEvent && ((ISharedObjectActivatedEvent) event).getActivatedID().equals(config.getSharedObjectID())) {
            synchronized (this) {
                if (dataGraph == null) {
                    try {
                        config.getContext().sendMessage(null, new RequestDataGraphMessage());
                    } catch (IOException e) {
        ...
        } else if (event instanceof ISharedObjectDeactivatedEvent && ((ISharedObjectDeactivatedEvent) event).getDeactivatedID().equals(config.getSharedObjectID())) {
            synchronized (this) {
                if (dataGraph != null && dataGraph.getChangeSummary().isLogging())
            ...
            if (msg instanceof RequestDataGraphMessage)
                ...
                handleReceiveDataGraphMessage(e.getRemoteContainerID(), m.getVersion(), m.getData());
            } else if (msg instanceof UpdateDataGraphMessage) {
                UpdateDataGraphMessage m = (UpdateDataGraphMessage) msg;
                handleUpdateDataGraphMessage(e.getRemoteContainerID(), m.getVersion(), m.getData());
            }
        }
    }
    ...
    private synchronized void handleRequestDataGraphMessage(ID containerID) {
        ...
        try {
            ...
            config.getContext().sendMessage(containerID, new ReceiveDataGraphMessage(version, data));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }

    private synchronized void handleReceiveDataGraphMessage(ID containerID, Version v, Object data) {
        if (dataGraph == null) {
            try {
    ...
    private synchronized void handleUpdateDataGraphMessage(ID containerID, Version v, Object data) {
        if (dataGraph == null)
            return;
        if (!v.equals(this.version)) {
            if (SDOPlugin.isTracing(TRACE_TAG))
                SDOPlugin.getTraceLog().println("Version mismatch: current=" + this.version + "; new=" + v);
