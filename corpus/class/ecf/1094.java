...
public class RegistrySharedObject extends BaseSharedObject implements IRemoteServiceContainerAdapter {
    ...
    protected Object callSynch(RemoteServiceRegistrationImpl registration, IRemoteCall call) throws ECFException {
        ...
        try {
            ...
            //$NON-NLS-1$
            Trace.trace(Activator.PLUGIN_ID, "callSync request sent with requestid=" + requestId);
            ...
            while ((timeout - System.currentTimeMillis()) > 0 && !doneWaiting) {
                synchronized (request) {
                    if (request.isDone()) {
                        ...
                        "callSynch.request/response done for requestId=" + //$NON-NLS-1$
                        requestId);
                        doneWaiting = true;
                        ...
                        //$NON-NLS-1$ //$NON-NLS-2$
                        Trace.trace(Activator.PLUGIN_ID, "Waiting " + RESPONSE_WAIT_INTERVAL + " for response to request: " + requestId);
    ...
    protected Request sendCallRequest(RemoteServiceRegistrationImpl remoteRegistration, final IRemoteCall call) throws IOException {
        ...
        //$NON-NLS-1$
        Trace.exiting(Activator.PLUGIN_ID, IRemoteServiceProviderDebugOptions.METHODS_EXITING, this.getClass(), "sendCallRequest", request);
        return request;
    }
