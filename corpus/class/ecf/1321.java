...
public class BasicTopologyManagerImpl extends AbstractTopologyManager implements EndpointEventListener {
    ...
    private static final boolean allowLoopbackReference = new Boolean(System.getProperty("org.eclipse.ecf.osgi.services.discovery.allowLoopbackReference", //$NON-NLS-1$
    "false")).booleanValue();
    ...
    "=*))";
     ...
     BasicTopologyManagerImpl(BundleContext context) {
        ...
        if (allowLoopbackReference)
            ...
            elScope.append("(&(!(").append(org.osgi.service.remoteserviceadmin.RemoteConstants.ENDPOINT_FRAMEWORK_UUID).append("=").append(getFrameworkUUID()).append("))");
    ...
    void handleRemoteAdminEvent(RemoteServiceAdminEvent event) {
        ...
        switch(eventType) {
            ...
            case RemoteServiceAdminEvent.EXPORT_ERROR:
                logError("handleRemoteAdminEvent.EXPORT_ERROR", //$NON-NLS-1$ //$NON-NLS-2$
                "Export error with event=" + rsaEvent);
                break;
            case RemoteServiceAdminEvent.EXPORT_WARNING:
                logWarning("handleRemoteAdminEvent.EXPORT_WARNING", //$NON-NLS-1$ //$NON-NLS-2$
                "Export warning with event=" + rsaEvent);
                break;
            ...
            case RemoteServiceAdminEvent.IMPORT_ERROR:
                logError("handleRemoteAdminEvent.IMPORT_ERROR", //$NON-NLS-1$//$NON-NLS-2$
                "Import error with event=" + rsaEvent);
                break;
            case RemoteServiceAdminEvent.IMPORT_WARNING:
                logWarning("handleRemoteAdminEvent.IMPORT_WARNING", //$NON-NLS-1$ //$NON-NLS-2$
                "Import warning with event=" + rsaEvent);
                break;
            default:
                logWarning("handleRemoteAdminEvent", //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
