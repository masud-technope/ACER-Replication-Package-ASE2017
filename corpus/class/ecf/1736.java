...
public class PublishedGraphTracker implements ISharedObject {
    ...
    private class Table {
        ...
        public synchronized ID[] getGraphs(ID containerID) {
            HashSet list = (HashSet) graphs.get(containerID);
            return list == null ? NO_GRAPHS : (ID[]) list.toArray(new ID[list.size()]);
        }
    ...
    public synchronized void add(ID graphID) throws ECFException {
        ...
        try {
            getContext().sendMessage(null, new Object[] { new Integer(ADD), graphID });
        } catch (IOException e) {
            throw new ECFException(e);
        }
        // track it yourself
    ...
    public void handleEvent(Event event) {
        ...
        // Ignore
            ...
            if (e.getTargetID().equals(getContext().getLocalContainerID()))
    ...
    private void handleJoined() {
        if (config.getHomeContainerID().equals(getContext().getLocalContainerID())) {
            // we're the primary copy -- replicate everywhere
            try {
                getContext().sendCreate(null, new ReplicaSharedObjectDescription(getClass(), config.getSharedObjectID(), config.getHomeContainerID()));
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
    ...
    private void handleJoined(ID containerID) {
        ...
        try {
            getContext().sendCreate(containerID, new ReplicaSharedObjectDescription(getClass(), config.getSharedObjectID(), config.getHomeContainerID(), props));
    ...
    private void handleRemoved(ID graphID) {
        try {
            getContext().sendMessage(null, new Object[] { new Integer(REMOVE), graphID });
        } catch (IOException e) {
            e.printStackTrace();
        }
