...
public class EndpointDiscoveryView extends ViewPart {
    ...
    private int invokeShowGroupBy(RegistryBrowser registryBrowser, int groupBy) throws NoSuchMethodException, SecurityException, IllegalAccessException, IllegalArgumentException, InvocationTargetException {
        return (int) registryBrowser.getClass().getDeclaredMethod(//$NON-NLS-1$
        "showGroupBy", //$NON-NLS-1$
        int.class).invoke(registryBrowser, groupBy);
    }
    ...
    protected int showInRegistryBrowser(int groupBy) {
        ...
        try {
            ...
            if (window != null) {
                ...
                if (page != null) {
                    IViewPart view = page.findView("org.eclipse.pde.runtime.RegistryBrowser");
                    if (view != null)
    ...
    protected EndpointNode createEndpointDescriptionNode(EndpointDescription ed) {
        EndpointNode edo = new EndpointNode(ed, new org.eclipse.ecf.remoteserviceadmin.ui.endpoint.model.ImportRegistrationNode(findImportRegistration(ed)));
        // Interfaces
