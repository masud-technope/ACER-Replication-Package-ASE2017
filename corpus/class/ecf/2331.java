...
public class SharedSDOEditor extends SDOEditor {
    ...
    private class UpdateConsumer implements IUpdateConsumer {
        ...
        public boolean consumeUpdate(ISharedDataGraph graph, ID containerID) {
            ChangeSummary changeSummary = graph.getDataGraph().getChangeSummary();
        ...
        public void updateFailed(ISharedDataGraph graph, ID containerID, Throwable cause) {
            EditorPlugin.getDefault().log(new CoreException(new Status(Status.ERROR, EditorPlugin.getDefault().getBundle().getSymbolicName(), 0, "Data graph upate failed.", cause)));
        }
    }
    ...
    protected boolean isConnected(IFileEditorInput editorInput) {
        try {
            EditorPlugin.getDefault().checkConnected(((IFileEditorInput) editorInput).getFile().getProject());
            return true;
    ...
    public void createModel() {
        try {
            ...
            if (isConnected(modelFile)) {
                ...
                URI uri = URI.createPlatformResourceURI(modelFile.getFile().getFullPath().toString());
                if (EditorPlugin.getDefault().isPublished(path)) {
                    sharedDataGraph = EditorPlugin.getDefault().subscribe(path, new UpdateConsumer());
                    EDataGraph dataGraph = (EDataGraph) sharedDataGraph.getDataGraph();
                    dataGraph.getDataGraphResource().setURI(uri);
                    editingDomain.getResourceSet().getResources().addAll(dataGraph.getResourceSet().getResources());
                    ...
                    sharedDataGraph = EditorPlugin.getDefault().publish(path, dataGraph, new UpdateConsumer());
                }
            } else
                super.createModel();
        } catch (ECFException e) {
