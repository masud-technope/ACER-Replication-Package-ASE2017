        ...
        try {
            ...
            for (int i = 0, max = baselineApiComponents.length; i < max; i++) {
                ...
                if (!visitedApiComponentNames.remove(id)) {
                    // filters
                    if (!isFiltered(id)) {
                        IApiProblem problem = ApiProblemFactory.newApiProblem(id, null, new String[] { id }, new String[] { IApiMarkerConstants.MARKER_ATTR_HANDLE_ID, IApiMarkerConstants.API_MARKER_ATTR_ID }, new Object[] { id, new Integer(IApiMarkerConstants.COMPATIBILITY_MARKER_ID) }, 0, -1, -1, IApiProblem.CATEGORY_COMPATIBILITY, IDelta.API_BASELINE_ELEMENT_TYPE, IDelta.REMOVED, IDelta.API_COMPONENT);
