...
public class LogTextPanel extends JPanel {
    ...
    void setTextColor(Priority p, String v) {
        StyleConstants.setForeground((MutableAttributeSet) fontAttributes.get(p), parseColor(v));
    }

    void setTextColor(Priority p, Color c) {
        StyleConstants.setForeground((MutableAttributeSet) fontAttributes.get(p), c);
    }
    ...
    void newEvents(EventBufferElement[] evts) {
        ...
        for (int i = 0; i < evts.length; i++) eventBuffer.add(evts[i]);
